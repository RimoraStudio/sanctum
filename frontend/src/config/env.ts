...
export const envConfig = {
  ...
  get CAPTCHA_SITE_KEY() {
    return (
      window?.__SANCTUM_RUNTIME_ENV__?.CAPTCHA_SITE_KEY || import.meta.env.VITE_CAPTCHA_SITE_KEY
    );
  },
  get INTERCOM_ID() {
    return window?.__SANCTUM_RUNTIME_ENV__?.INTERCOM_ID || import.meta.env.VITE_INTERCOM_ID;
  },
  get POSTHOG_API_KEY() {
    return (
      window?.__SANCTUM_RUNTIME_ENV__?.POSTHOG_API_KEY || import.meta.env.VITE_POSTHOG_API_KEY
    );
  },
  get POSTHOG_HOST() {
  ...
  get TELEMETRY_CAPTURING_ENABLED() {
    return (
      window?.__SANCTUM_RUNTIME_ENV__?.TELEMETRY_CAPTURING_ENABLED ||
  ...
  get PLATFORM_NAME() {
    return (
      window?.__SANCTUM_RUNTIME_ENV__?.PLATFORM_NAME ||
      import.meta.env.VITE_PLATFORM_NAME ||
      "Sanctum"
    );
  },
  get PLATFORM_VERSION() {
    // Release tags (e.g. "v0.159.15") are passed through as SANCTUM_PLATFORM_VERSION
    // by the release workflow. Strip any leading "v" so consumers can prefix it
    // themselves without producing "vv0.159.15".
    return import.meta.env.VITE_SANCTUM_PLATFORM_VERSION?.replace(/^v/i, "");
  }
};
