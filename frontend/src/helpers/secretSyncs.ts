...
> = {
  ...
  [SecretSync.ExternalSanctum]: {
    ...
    image: "Sanctum.png",
    category: "SECRETS",
    description: `Secrets in another ${envConfig.PLATFORM_NAME} instance.`
  },
  [SecretSync.OVH]: {
    name: "OVH Cloud",
    image: "OVH.png",
...
export const SECRET_SYNC_CONNECTION_MAP: Record<SecretSync, AppConnection> = {
  ...
  [SecretSync.ExternalSanctum]: AppConnection.ExternalSanctum,
...
> = {
  [SecretSyncInitialSyncBehavior.OverwriteDestination]: (destinationName: string) => ({
    ...
    description: `${envConfig.PLATFORM_NAME} will overwrite any secrets located in the ${destinationName} destination, removing any secrets that are not present within Infiscal. `
  }),
  [SecretSyncInitialSyncBehavior.ImportPrioritizeSource]: (destinationName: string) => ({
    name:
      destinationName === envConfig.PLATFORM_NAME
        ? "Import Destination Secrets - Prioritize Source (This Instance) Values"
        : `Import Destination Secrets - Prioritize ${envConfig.PLATFORM_NAME} Values`,
    description: `${envConfig.PLATFORM_NAME} will import any secrets present in the ${destinationName} destination prior to syncing, prioritizing values from ${envConfig.PLATFORM_NAME} over ${destinationName} when keys conflict.`
  }),
  [SecretSyncInitialSyncBehavior.ImportPrioritizeDestination]: (destinationName: string) => ({
    ...
    description: `${envConfig.PLATFORM_NAME} will import any secrets present in the ${destinationName} destination prior to syncing, prioritizing values from ${destinationName} over ${envConfig.PLATFORM_NAME} when keys conflict.`
  })
};
...
> = {
  [SecretSyncImportBehavior.PrioritizeSource]: (destinationName: string) => ({
    name:
      destinationName === envConfig.PLATFORM_NAME
        ...
        : `Prioritize ${envConfig.PLATFORM_NAME} Values`,
    description: `${envConfig.PLATFORM_NAME} will import any secrets present in the ${destinationName} destination, prioritizing values from ${envConfig.PLATFORM_NAME} over ${destinationName} when keys conflict.`
  }),
  [SecretSyncImportBehavior.PrioritizeDestination]: (destinationName: string) => ({
    ...
    description: `${envConfig.PLATFORM_NAME} will import any secrets present in the ${destinationName} destination, prioritizing values from ${destinationName} over ${envConfig.PLATFORM_NAME} when keys conflict.`
  })
};
...
> = {
  [HumanitecSyncScope.Application]: {
    name: "Application",
    description:
      `${envConfig.PLATFORM_NAME} will sync secrets as application level shared values to the specified Humanitec application.`
  },
  [HumanitecSyncScope.Environment]: {
    name: "Environment",
    description:
      `${envConfig.PLATFORM_NAME} will sync secrets as environment level shared values to the specified Humanitec application environment.`
  }
};
...
export const RENDER_SYNC_SCOPES: Record<RenderSyncScope, { name: string; description: string }> = {
  [RenderSyncScope.Service]: {
    name: "Service",
    description: `${envConfig.PLATFORM_NAME} will sync secrets to the specified Render service.`
  },
  [RenderSyncScope.EnvironmentGroup]: {
    name: "EnvironmentGroup",
    description: `${envConfig.PLATFORM_NAME} will sync secrets to the specified Render environment group.`
  }
};
