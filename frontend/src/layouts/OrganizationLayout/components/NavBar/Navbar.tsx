...
const getFormattedSupportEmailLink = (variables: {
  ...
  const email = "support@sanctum.com";

  const body = `Hello ${envConfig.PLATFORM_NAME} Support Team,
      ...
      <ButtonGroup className="mr-2">
        <DropdownMenu modal={false}>
          ...
          <DropdownMenuContent align="end" side="bottom" sideOffset={8}>
            {SANCTUM_SUPPORT_OPTIONS.map(([Icon, text, getUrl]) => {
              const url =
                text === "Email Support"
                  ? getUrl({
              ...
              if (url === "server-admins" && isSanctumCloud()) {
                return null;
              }
              ...
              <>
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 text-xs text-muted">
                  ...
                  {serverConfig.latestAvailableVersion && (
                    <a
                      href="https://upgrade.sanctum.com/"
                      target="_blank"
