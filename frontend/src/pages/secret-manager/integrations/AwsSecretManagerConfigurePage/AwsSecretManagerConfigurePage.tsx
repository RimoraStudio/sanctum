...
const mappingBehaviors = [
  {
    label: `Many to One (All ${envConfig.PLATFORM_NAME} secrets will be mapped to a single AWS secret)`,
    value: IntegrationMappingBehavior.MANY_TO_ONE
  },
  {
    label: `One to One - (Each ${envConfig.PLATFORM_NAME} secret will be mapped to its own AWS secret)`,
      ...
      <form onSubmit={handleSubmit(handleButtonClick)}>
        <Card className="max-w-lg rounded-md border border-mineshaft-600">
          <CardTitle
            ...
            subTitle={`Choose which environment in ${envConfig.PLATFORM_NAME} you want to sync to secerts in AWS Secrets Manager.`}
          >
                ...
                <Controller
                  ...
                  render={({ field, fieldState: { error } }) => (
                    <FormControl
                      ...
                      className="mt-4"
                    >
                      <Input placeholder="SANCTUM_" {...field} />
                    </FormControl>
                  )}
                />
