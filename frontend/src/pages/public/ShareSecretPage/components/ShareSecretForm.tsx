    ...
    if (processedEmails && processedEmails.length > 0) {
      ...
      createNotification({
        text: showAccountRequiredMessage
          ? `If the provided ${processedEmails.length > 1 ? "emails are" : "email is"} associated with an ${envConfig.PLATFORM_NAME} account they will receive a link`
                    ...
                    <Controller
                      ...
                      render={({ field, fieldState: { error } }) => (
                        <Field>
                              ...
                              <TooltipContent className="max-w-sm">
                                ...
                                <p className="mt-2">
                                  {isAllowingExternalEmails
                                    ? `External recipients (user without an ${envConfig.PLATFORM_NAME} account) will need the password to view the secret. Authorized recipients will also need a password if the secret is password-protected.`
                                    : `Recipients must have an ${envConfig.PLATFORM_NAME} account to verify their identity.`}
                                </p>
                              </TooltipContent>
                            </Tooltip>
                          </FieldLabel>
                          ...
                          <FieldDescription>
                            {isAllowingExternalEmails
                              ? `All recipients will receive a link and need the password to access the secret. They don't need an ${envConfig.PLATFORM_NAME} account, but a password is mandatory in this case.`
                              : `Recipients must have an ${envConfig.PLATFORM_NAME} account to verify identity`}
                          </FieldDescription>
                          {error && <FieldError>{error.message}</FieldError>}
                        </Field>
                      )}
        ...
        <div className="flex w-full justify-end">
          {isPublic && (
            <Badge variant="ghost" className="mt-auto mr-auto">
              <img src="/sanctum-logo.svg" alt={envConfig.PLATFORM_NAME} className="mr-1 h-3" />
              Powered by {envConfig.PLATFORM_NAME}
            </Badge>
          )}
          <Button
            size="md"
