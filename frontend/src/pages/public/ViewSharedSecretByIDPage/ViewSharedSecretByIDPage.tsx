...
export const DEFAULT_FAVICON_URL = "/sanctum.ico";
...
export const ViewSharedSecretByIDPage = () => {
  ...
  // Custom branding: minimal layout with brand colors, no Sanctum elements
  if (hasCustomBranding) {
    return (
      <>
        <Helmet>
  ...
  // Default: Sanctum auth page layout
  return (
    <div className="relative flex max-h-screen min-h-screen flex-col overflow-y-auto bg-bunker-800 px-4 text-foreground scheme-dark">
      <AuthPageBackground />
      <Helmet>
        <title>Securely Share Secrets | {envConfig.PLATFORM_NAME}</title>
        <meta property="og:image" content="/images/message.png" />
      </Helmet>
      <AuthPageHeader />
