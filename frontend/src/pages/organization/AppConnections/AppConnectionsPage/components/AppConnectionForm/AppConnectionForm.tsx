...
import { ExternalSanctumConnectionForm } from "./ExternalSanctumConnectionForm";
    ...
    <div className="p-4">
      <div className="flex flex-col rounded-xs border border-l-2 border-mineshaft-600 border-l-primary bg-mineshaft-700/80 px-4 py-3">
        ...
        <p className="bor mt-1 text-sm text-bunker-200">
          Enabling automatic credential rotation will give {envConfig.PLATFORM_NAME} full control over the lifecycle
          of this credential. {envConfig.PLATFORM_NAME} will automatically rotate the credential on the schedule you
...
const CreateForm = ({ app, onComplete, projectId }: CreateFormProps) => {
  ...
  const renderForm = () => {
    switch (app) {
      ...
      case AppConnection.ExternalSanctum:
        return <ExternalSanctumConnectionForm onSubmit={onSubmit} />;
...
const UpdateForm = ({ appConnection, onComplete }: UpdateFormProps) => {
  ...
  const renderForm = () => {
    switch (appConnection.app) {
      ...
      case AppConnection.ExternalSanctum:
        return (
          <ExternalSanctumConnectionForm onSubmit={onSubmit} appConnection={appConnection} />
        );
      case AppConnection.Doppler:
