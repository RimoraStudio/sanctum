...
export const GitHubConnectionForm = ({ appConnection, projectId, onSubmit }: Props) => {
  ...
  // Resolve the selected gateway/pool name so the create-app modal can tell the user how Sanctum
  ...
  const getMethodErrorText = (fieldError?: { message?: string }) => {
    if (!isLoading && isMissingConfig) {
      const configHint = isSanctumCloud()
        ? `Please contact ${envConfig.PLATFORM_NAME}.`
    ...
    if (isCloudCustomHostUnsupported) {
      return `GitHub App/OAuth with a custom host is only supported on self-hosted ${envConfig.PLATFORM_NAME}.`;
    }
