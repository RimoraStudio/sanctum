...
export const RelayDeployCommandContent = ({
  ...
  const cliCommand = useMemo(() => {
    if (authMethod === "aws") {
      return `sanctum relay start --name=${relayName} --enroll-method=aws --relay-id=${relayId} --domain=${siteURL}`;
    }
    return `sanctum relay start --name=${relayName} --enroll-method=token --token=${enrollmentToken} --domain=${siteURL}`;
  }, [relayName, relayId, enrollmentToken, authMethod, siteURL]);

  const systemdInstallCommand = useMemo(() => {
    if (authMethod === "aws") {
      return `sudo sanctum relay systemd install ${relayName} --enroll-method=aws --relay-id=${relayId} --domain=${siteURL}`;
    }
    return `sudo sanctum relay systemd install ${relayName} --enroll-method=token --token=${enrollmentToken} --domain=${siteURL}`;
  }, [relayName, relayId, enrollmentToken, authMethod, siteURL]);
