...
export const KubernetesStartCommandContent = ({ gatewayId, gatewayName }: Props) => {
  ...
  const helmCommand = useMemo(() => {
    const relayPart = resolvedRelayName
      ...
      : "";
    return `helm repo add sanctum https://dl.cloudsmith.io/public/sanctum/helm-charts/helm/charts/
helm install sanctum-gateway sanctum/sanctum-gateway \\
  --namespace sanctum-gateway --create-namespace \\
  ...
  const cliCommand = useMemo(() => {
    ...
    return `sanctum gateway start ${gatewayName} --enroll-method=kubernetes --gateway-id=${gatewayId}${relayPart} --domain=${siteURL}`;
  }, [gatewayName, gatewayId, resolvedRelayName, siteURL]);
