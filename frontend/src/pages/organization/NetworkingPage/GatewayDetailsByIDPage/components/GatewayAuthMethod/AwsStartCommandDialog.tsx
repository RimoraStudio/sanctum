...
export const AwsStartCommandContent = ({ gatewayId, gatewayName }: Props) => {
  ...
  const cliCommand = useMemo(() => {
    ...
    return `sanctum gateway start ${gatewayName} --enroll-method=aws --gateway-id=${gatewayId}${relayPart} --domain=${siteURL}`;
  }, [gatewayName, gatewayId, resolvedRelayName, siteURL]);

  const systemdInstallCommand = useMemo(() => {
    const relayPart = resolvedRelayName ? ` --target-relay-name=${resolvedRelayName}` : "";
    return `sudo sanctum gateway systemd install ${gatewayName} --enroll-method=aws --gateway-id=${gatewayId}${relayPart} --domain=${siteURL}`;
  }, [gatewayName, gatewayId, resolvedRelayName, siteURL]);
