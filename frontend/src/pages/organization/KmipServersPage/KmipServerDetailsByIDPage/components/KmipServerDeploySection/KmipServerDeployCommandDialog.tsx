...
export const KmipServerDeployCommandDialog = ({
  ...
  const cliCommand = useMemo(() => {
    ...
    if (authMethod === "aws") {
      return `sanctum kmip start ${kmipServerName} --enroll-method=aws --kmip-server-id=${kmipServerId} ${common}`;
    }
    return `sanctum kmip start ${kmipServerName} --enroll-method=token --token=${enrollmentToken} ${common}`;
  }, [kmipServerName, kmipServerId, enrollmentToken, authMethod, siteURL]);

  const systemdInstallCommand = useMemo(() => {
    const common = `--domain=${siteURL}`;
    if (authMethod === "aws") {
      return `sudo sanctum kmip systemd install ${kmipServerName} --enroll-method=aws --kmip-server-id=${kmipServerId} ${common}`;
    }
    return `sudo sanctum kmip systemd install ${kmipServerName} --enroll-method=token --token=${enrollmentToken} ${common}`;
  }, [kmipServerName, kmipServerId, enrollmentToken, authMethod, siteURL]);

  const startServiceCommand = "sudo systemctl start sanctum-kmip";
  ...
  return (
    <Modal isOpen={isOpen} onOpenChange={onOpenChange}>
      ...
      >
        ...
        >
          <span>Install the {envConfig.PLATFORM_NAME} CLI</span>
