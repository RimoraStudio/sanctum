...
export const ProjectCategoryOverview = () => {
  ...
  const secretManagerProjects = projects.filter((p) => p.type === ProjectType.SecretManager);
  const secondaryTypes = PRODUCT_TYPES.filter((t) => t !== ProjectType.SecretManager);
  ...
  const smStyles = PRODUCT_STYLES[ProjectType.SecretManager];
  const SmIcon = getProjectLucideIcon(ProjectType.SecretManager);

  return (
    <>
      {/* Primary surface: Secrets Management with direct project access */}
      <Card className="mb-5 rounded-md">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              <div
                className={`shrink-0 rounded-sm border p-1.5 ${smStyles.containerClassName}`}
              >
                <SmIcon className={smStyles.iconClassName} />
              </div>
              <div className="min-w-0">
                <div className="text-base font-semibold text-foreground">
                  {getProjectTitle(ProjectType.SecretManager)}
                </div>
                <p className="text-xs text-muted">
                  {getProjectDescription(ProjectType.SecretManager)}
                </p>
              </div>
            </div>
            <Link
        ...
        <CardContent className="pt-0">
          {secretManagerProjects.length > 0 ? (
            <div className="divide-y divide-border border-t border-border">
              {secretManagerProjects.map((project) => (
                <Link
                  key={project.id}
                  to="/organizations/$orgId/projects/secret-management/$projectId/overview"
                  params={{ orgId: currentOrg?.id || "", projectId: project.id }}
                  className="group flex items-center gap-3 py-2.5 transition-colors hover:bg-foreground/[0.02]"
                >
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-sm border border-border bg-container text-xs font-medium uppercase text-muted group-hover:border-product-sm/40 group-hover:text-product-sm">
                  ...
                  <ArrowRightIcon className="h-3.5 w-3.5 shrink-0 text-muted opacity-0 transition-opacity group-hover:opacity-100" />
                </Link>
              ))}
            </div>
          ) : (
            <div className="flex items-center justify-between gap-3 rounded-md border border-dashed border-border px-4 py-3">
              <span className="text-sm text-muted">No secrets projects yet.</span>
              <Link
                to="/organizations/$orgId/projects/$type"
                params={{ orgId: currentOrg?.id || "", type: "secret-management" }}
                className="flex shrink-0 items-center gap-1 text-xs text-foreground transition-colors hover:text-product-sm"
              >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {secondaryTypes.map((type) => {
          const stats = getStatsForType(type).filter((s) => s.value > 0);
          const { iconClassName, containerClassName, cardClassName } = PRODUCT_STYLES[type];
              ...
              <div className="min-w-0 flex-1">
                ...
                {stats.length > 0 ? (
                  <div className="truncate text-xs text-muted">
                    {stats.map((s) => `${formatNumber(s.value)} ${s.label}`).join(" · ")}
                  </div>
                ) : (
                  <div className="truncate text-xs text-muted">
              ...
              <Tooltip key={type}>
                <TooltipTrigger asChild>
                  <div aria-disabled className="cursor-not-allowed">
                    <Card className="h-auto rounded-md p-0 opacity-50">{tileBody}</Card>
                  </div>
                </TooltipTrigger>
                <TooltipContent>
          ...
          const tileClassName = `group h-auto cursor-pointer rounded-md p-0 transition-colors duration-200 hover:border-border/80 ${cardClassName}`;
