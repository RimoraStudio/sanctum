...
const FLOW_OPTIONS = [
  ...
  },
  {
    ...
    rfc: "RFC 8693",
    description:
      `The application presents a user's token from your identity provider and receives that user's ${envConfig.PLATFORM_NAME} token. No browser and no consent screen, and the token carries that user's full permissions.`
  }
] as const;
      ...
      <SheetContent className="sm:max-w-xl">
        <form onSubmit={handleSubmit(onFormSubmit)} className="flex h-full min-h-0 flex-col">
          <SheetHeader>
            ...
            <SheetDescription>
              External platforms use this application to request delegated access to {envConfig.PLATFORM_NAME} on a
                    ...
                    render={({ field, fieldState: { error } }) => (
                      <Field>
                        ...
                        <TextArea
                          ...
                          placeholder="https://coder.example.com/external-auth/sanctum/callback"
                          rows={3}
                          {...field}
                        />
                        <FieldDescription>
                    ...
                    render={({ field: { value, onChange } }) => (
                      <Field orientation="horizontal">
                        <FieldContent>
                          ...
                          <FieldDescription>
                            Token exchange has no {envConfig.PLATFORM_NAME} MFA challenge to run. Turn this on to
