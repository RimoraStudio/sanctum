  ...
  const handleSamlSSOToggle = async (value: boolean) => {
    ...
    if (!subscription?.samlSSO) {
      handlePopUpOpen("upgradePlan", {
        text: `Your current plan does not include access to SAML SSO. To unlock this feature, please upgrade to ${envConfig.PLATFORM_NAME} Pro plan.`
      });
      return;
    }
  ...
  const handleSamlGroupManagement = async (value: boolean) => {
    ...
    if (!subscription?.samlSSO || !subscription?.groups) {
      handlePopUpOpen("upgradePlan", {
        ...
        text: `Your current plan does not include access to SAML group mapping. To unlock this feature, please upgrade to ${envConfig.PLATFORM_NAME} Enterprise plan.`
      });
      return;
    }
          ...
          <FieldGroup>
            <Field orientation="horizontal">
              <FieldContent>
                ...
                <FieldDescription>
                  Allow members to authenticate into {envConfig.PLATFORM_NAME} with SAML.
                </FieldDescription>
              </FieldContent>
              {!isPending && (
                <OrgPermissionCan
            ...
            {data && GROUP_SYNC_SUPPORTED_PROVIDERS.includes(data.authProvider) && (
              <Field orientation="horizontal">
                <FieldContent>
                  <FieldTitle>
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent className="max-w-lg">
                        <p>
                          When this feature is enabled, {envConfig.PLATFORM_NAME} will automatically sync group
                          memberships between the SAML provider and {envConfig.PLATFORM_NAME}. Users will be added
                          to {envConfig.PLATFORM_NAME} groups that match their SAML group names.
                        </p>
                        <p className="mt-3">
                        ...
                        <p className="mt-3 flex items-start gap-1.5 text-warning">
                          ...
                          <span>
                            Group membership changes in the SAML provider only sync with {envConfig.PLATFORM_NAME}
                            when a user logs in via SAML — removing a user from a group in the SAML
                            provider will not be reflected in {envConfig.PLATFORM_NAME} until their next SAML login.
                  ...
                  <FieldDescription>
                    {envConfig.PLATFORM_NAME} will manage user group memberships based on the SAML provider.
                  </FieldDescription>
                </FieldContent>
