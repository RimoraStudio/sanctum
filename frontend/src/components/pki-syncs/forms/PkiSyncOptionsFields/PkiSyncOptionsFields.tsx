    ...
    <>
      {syncOption?.canRemoveCertificates && (
        <SyncSwitchField
          ...
          description={
            <>
              When enabled, {envConfig.PLATFORM_NAME} removes certificates from the destination during a sync once
              they are no longer active in {envConfig.PLATFORM_NAME}. Disable this if you manage some certificates
              manually outside of {envConfig.PLATFORM_NAME}.
          ...
          render={({ field: { value, onChange }, fieldState: { error } }) => (
            <Field className="mb-4">
              ...
              <Input
                ...
                placeholder={
                  syncOption?.defaultCertificateNameSchema || "SANCTUM_{{certificateId}}"
                }
                isError={Boolean(error)}
              />
              <FieldDescription>
                {currentDestination === PkiSync.AwsElasticLoadBalancer
                  ? `Set a Certificate Name Schema so ${envConfig.PLATFORM_NAME} only manages the specific certificates you intend to, keeping everything else untouched.`
                  : `The Certificate Name Schema ensures ${envConfig.PLATFORM_NAME} only manages the specific certificates you intend to, keeping everything else untouched.`}
              </FieldDescription>
