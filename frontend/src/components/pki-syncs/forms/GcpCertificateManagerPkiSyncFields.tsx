...
export const GcpCertificateManagerPkiSyncFields = ({ isUpdate }: Props) => {
  ...
  const certificateMapBindingHelpText = (() => {
    ...
    return `${envConfig.PLATFORM_NAME} keeps a certificate map entry pointed at this sync's certificate, so renewals need no change in GCP. Leave this off to upload certificates only.`;
  })();
            ...
            render={({ field: { value, onChange }, fieldState: { error } }) => {
              ...
              return (
                <Field className="mb-4">
                  ...
                  <FieldDescription>
                    {envConfig.PLATFORM_NAME} creates entries inside this map but never creates or deletes the map
                    itself.
                  </FieldDescription>
                  <FieldError errors={[combinedError]} />
                </Field>
