...
type GraphicVariant = "overwrite" | "prioritize-sanctum" | "prioritize-destination";
...
const getGraphicVariant = (key: string): GraphicVariant => {
  ...
  if (key === SecretSyncInitialSyncBehavior.ImportPrioritizeSource) return "prioritize-sanctum";
  ...
  switch (key) {
    case SecretSyncInitialSyncBehavior.OverwriteDestination:
      return {
        ...
        description: `On the initial sync, ${envConfig.PLATFORM_NAME} will write its secrets to ${destinationName}. No secrets will be imported from ${shortName}. Depending on your configuration this can lead to removal of secrets from ${shortName}.`
      };
    case SecretSyncInitialSyncBehavior.ImportPrioritizeSource:
      return {
        title: `Import from ${shortName} — prioritize ${envConfig.PLATFORM_NAME}`,
        description: `Prior to the initial sync, ${envConfig.PLATFORM_NAME} will import secrets from ${destinationName}. If an imported secret already exists in ${envConfig.PLATFORM_NAME}, the imported secret value is ignored, preserving the ${envConfig.PLATFORM_NAME} value.`
      };
    case SecretSyncInitialSyncBehavior.ImportPrioritizeDestination:
    default:
      return {
        title: `Import from ${shortName} — prioritize ${shortName}`,
        description: `Prior to the initial sync, ${envConfig.PLATFORM_NAME} will import secrets from ${destinationName}. If an imported secret already exists in ${envConfig.PLATFORM_NAME}, the imported secret value will overwrite the existing value in ${envConfig.PLATFORM_NAME}.`
      };
  }
};
...
const BEFORE_SANCTUM: ReconciliationRow[] = [{ name: "API_KEY" }, { name: "DB_URL" }];
const BEFORE_DESTINATION: ReconciliationRow[] = [{ name: "API_KEY" }, { name: "LEGACY_TOKEN" }];

const getAfterRows = (
  variant: GraphicVariant
): { sanctum: ReconciliationRow[]; destination: ReconciliationRow[] } => {
  switch (variant) {
    case "overwrite":
      return {
        sanctum: [{ name: "API_KEY" }, { name: "DB_URL" }],
        destination: [
  ...
  return (
    ...
    !keySchema && (
      <Alert variant="warning">
        ...
        <AlertDescription>
          <p>
            Anything in {destinationName} not in {envConfig.PLATFORM_NAME} will be removed. To keep them,{" "}
        ...
        <Alert className="mb-3" variant="warning">
          ...
          <AlertDescription>
            ...
            {!disableSecretDeletion &&
              ...
              ` Secrets not in ${envConfig.PLATFORM_NAME} will be removed — add a key schema or disable secret deletion to keep them.`}
          </AlertDescription>
        </Alert>
      )}
      {vercelSensitive && (
        <Alert className="mb-3" variant="warning">
          ...
          <AlertDescription>
            Vercel can&apos;t read sensitive secrets back, so {envConfig.PLATFORM_NAME} must overwrite the
            destination.
          </AlertDescription>
        </Alert>
      )}
