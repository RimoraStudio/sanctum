...
import {
  ...
  useExternalSanctumConnectionGetEnvironmentFolderTree,
  useExternalSanctumConnectionListProjects
} from "@app/hooks/api/appConnections/external-sanctum";
import { envConfig } from "@app/config/env";
import { SecretSync } from "@app/hooks/api/secretSyncs";

import { TSecretSyncForm } from "../schemas";

export const ExternalSanctumSyncFields = () => {
  const { control, setValue } = useFormContext<
    TSecretSyncForm & { destination: SecretSync.ExternalSanctum }
  >();
  ...
  const { data: projects = [], isPending: isProjectsLoading } =
    useExternalSanctumConnectionListProjects(connectionId, {
      enabled: Boolean(connectionId)
    });

  const { data: folderTree = {} } = useExternalSanctumConnectionGetEnvironmentFolderTree(
    connectionId,
    projectId,
  ...
  const childFolderNamesAtPath = useMemo(() => {
    ...
    const tree = folderTree as TRemoteSanctumEnvironmentFolderTree;
    const envData =
          ...
          <Field>
            <FieldLabel>
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  The project on the remote {envConfig.PLATFORM_NAME} instance to sync secrets to. Ensure the
            ...
            <FieldContent>
              <FilterableSelect
                ...
                onChange={(option) => {
                  const v = option as SingleValue<TRemoteSanctumProject>;
                  onChange(v?.id ?? "");
                  setValue("destinationConfig.environment", "");
                  setValue("destinationConfig.secretPath", "/");
                }}
          ...
          <Field>
            <FieldLabel>
              ...
              <Tooltip>
                ...
                <TooltipContent>The environment on the remote {envConfig.PLATFORM_NAME} instance.</TooltipContent>
              </Tooltip>
            </FieldLabel>
            <FieldContent>
              <FilterableSelect
          ...
          <Field>
            <FieldLabel>
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  The folder path on the remote {envConfig.PLATFORM_NAME} instance to sync secrets to.
                </TooltipContent>
              </Tooltip>
            </FieldLabel>
            <FieldContent>
