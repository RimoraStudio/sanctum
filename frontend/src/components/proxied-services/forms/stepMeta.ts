...
export const PROXIED_SERVICE_STEPS: StepMeta[] = [
  ...
  },
  {
    ...
    rightDescription:
      `${envConfig.PLATFORM_NAME} hands the agent a placeholder as an environment variable; the proxy swaps it for the real secret wherever it appears in the surfaces you pick (path, query, header, body). Ideal when the credential sits in the URL or the agent's client validates its format.`
  },
  {
