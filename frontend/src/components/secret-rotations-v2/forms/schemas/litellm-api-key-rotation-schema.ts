...
/** Options owned by Sanctum or set via dedicated fields; users cannot set these via the additional options JSON. */
      ...
      additionalOptions: z
        ...
        .superRefine((val, ctx) => {
          ...
          if (reservedKeys.length) {
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
              message: `The following options are managed by ${envConfig.PLATFORM_NAME} and cannot be set: ${reservedKeys.join(", ")}.`
            });
          }
        }),
