          ...
          render={({ field: { value, onChange, onBlur, ref }, fieldState: { error } }) => (
            <Field data-invalid={Boolean(error)}>
              <FieldLabelWithTooltip
                ...
                tooltip={`A descriptive name for the generated key. ${envConfig.PLATFORM_NAME} appends a timestamp so each rotated key stays unique and records its creation time.`}
              >
                Key Name
              </FieldLabelWithTooltip>
              <Input
      ...
      <TabsContent value={ParameterTab.Advanced} className="space-y-4">
        <Controller
          ...
          render={({ field, fieldState: { error } }) => (
            <Field data-invalid={Boolean(error)}>
              <FieldLabelWithTooltip tooltip={`Common fields: max_budget (number), tpm_limit / rpm_limit (numbers), metadata (object). user_id, team_id and models are set on the general tab; key_alias, auto_rotate, rotation_interval, duration, send_invite_email and key_type are managed by ${envConfig.PLATFORM_NAME} and cannot be set here.`}>
