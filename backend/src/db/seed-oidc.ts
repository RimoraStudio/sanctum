...
const main = async () => {
  ...
  const clientId = process.env.SEED_OIDC_CLIENT_ID || "sanctum-dev";
  const clientSecret = process.env.SEED_OIDC_CLIENT_SECRET || "sanctum-dev-client-secret";
  ...
  const discoveryURL =
    process.env.SEED_OIDC_DISCOVERY_URL || "http://keycloak:8080/realms/sanctum/.well-known/openid-configuration";
