...
// userName, and Sanctum only allows SCIM once an SSO config exists). It orchestrates BOTH sides:
//   * Sanctum (DB): a `saml` org (scimEnabled), a login-capable `admin@saml.com` admin, a verified
...
//     provider (ACS -> Sanctum, NameID = email, audience spn:SITE_URL for SP-initiated), a SCIM
//     provider pointed at Sanctum's SCIM endpoint with that token, and an application binding both.
//   * A SCIM sync is triggered at the end (provider save), provisioning the group into Sanctum.
//
...
// Fixed so the Sanctum SAML config id (= the Authentik ACS path) is stable across re-runs.
...
const GROUP_NAME = "sanctum-saml";
const APP_SLUG = "sanctum";
const TEST_USERS = [
...
// where Authentik's SCIM sync calls Sanctum back (compose-internal).
...
const SANCTUM_SCIM_URL = process.env.SEED_SCIM_URL || "http://backend:4000/api/v1/scim";
...
const main = async () => {
  ...
  try {
    ...
    // ---- Sanctum: login-capable admin (password login + org owner) ----
    ...
    // ---- Sanctum: verified email domain (required for SCIM POST + SAML login) ----
    ...
    await db(TableName.EmailDomains).insert({
      ...
      verificationRecordName: `_sanctum-verification.${DOMAIN}`,
      status: EmailDomainStatus.Verified,
      verifiedAt: new Date(),
      codeExpiresAt: new Date(Date.now() + ONE_YEAR_MS)
    });

    // ---- Sanctum: SCIM token (scim_tokens row + JWT signed with AUTH_SECRET, mirrors scim-service) ----
    // Regenerated each run; the Authentik SCIM provider is re-PATCHed with it below so the two never
    // drift (e.g. after an Sanctum DB reset that leaves Authentik holding a now-dangling token).
    ...
    // Custom SAML attribute mappings emitting the exact attribute names Sanctum reads (`email`,
    ...
    const emailMapping = await ensureSamlMapping("Sanctum: email", "email", "return request.user.email");
    ...
    // ---- Authentik: SAML provider (ACS -> Sanctum, NameID = email) ----
    const samlBody = {
      name: "sanctum-saml",
      authorization_flow: authFlow,
      invalidation_flow: invalidationFlow,
      acs_url: `${SITE_URL}/api/v1/sso/saml2/${SAML_CONFIG_ID}`,
      // node-saml validates the assertion audience against SITE_URL (the SP entityID / Sanctum
      // `issuer`). Sanctum only expects the `spn:${issuer}` variant when its spInitiated RelayState
    ...
    let samlProviderPk = await akFindPk(`/providers/saml/?name=sanctum-saml`);
    if (samlProviderPk) {
    ...
    // ---- Authentik: read the SAML signing cert (for the Sanctum SAML config) ----
    ...
    // ---- Sanctum: active SAML config (entryPoint/issuer/cert KMS-encrypted; unblocks SCIM) ----
    ...
    // ---- Authentik: SCIM provider -> Sanctum (users + groups; service accounts excluded) ----
    ...
    // each provisioned user/group); after an Sanctum DB reset those point at rows that no longer
    ...
    const deleteScimProvider = async () => {
      const pk = await akFindPk(`/providers/scim/?name=sanctum-scim`);
    ...
    // Wait for the Sanctum backend HTTP server to be reachable (it may still be booting after
    ...
    const healthUrl = `${SANCTUM_SCIM_URL.replace(/\/api\/v1\/scim$/, "")}/api/status`;
    let backendUp = false;
    ...
    if (!backendUp) logger.warn(`Sanctum backend not reachable at ${healthUrl}; SCIM sync may not provision.`);

    // ---- Provision + verify, with retries. Touch each test user to fire Authentik's per-object SCIM
    // sync (the user + their groups, i.e. sanctum-saml). The sync is async, so poll Sanctum and
    ...
    for (
      ...
      // rounds until the users it references exist in Sanctum).
      // eslint-disable-next-line no-await-in-loop
      await Promise.all([
        ...userPks.map((pk) => akSend("PATCH", `/core/users/${pk}/`, { attributes: { sanctumResync: round } })),
        akSend("PATCH", `/core/groups/${groupPk}/`, { attributes: { sanctumResync: round } })
      ]);
      // eslint-disable-next-line no-await-in-loop
      await new Promise((resolve) => {
    ...
    logger.info(
      `SAML/SCIM seeded [org=${org.slug}]: admin '${ADMIN_EMAIL}', verified '${DOMAIN}', SAML entryPoint ${SAML_ENTRY_POINT}, SCIM -> ${SANCTUM_SCIM_URL}; provisioning ${TEST_USERS.map(
        (u) => u.email
      ).join(", ")}`
    );
  } finally {
