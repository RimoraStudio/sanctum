...
export const registerLoginRouter = async (server: FastifyZodProvider) => {
  ...
  server.route({
    ...
    handler: async (req, res) => {
      ...
      const githubOauthAccessToken = req.cookies[SANCTUM_PROVIDER_GITHUB_ACCESS_TOKEN];
      if (githubOauthAccessToken) {
        await server.services.githubOrgSync
          ...
          .finally(() => {
            void res.setCookie(SANCTUM_PROVIDER_GITHUB_ACCESS_TOKEN, "", {
              httpOnly: true,
              path: "/api",
              sameSite: "strict",
      ...
      void res.cookie("sanctum-project-assume-privileges", "", {
        httpOnly: true,
        path: "/api",
        sameSite: "strict",
        secure: cfg.HTTPS_ENABLED,
