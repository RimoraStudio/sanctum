...
export const registerRoutes = async (
  ...
  const shouldForwardWritesToPrimaryInstance = Boolean(envConfig.SANCTUM_PRIMARY_INSTANCE_URL);
  if (shouldForwardWritesToPrimaryInstance) {
    logger.info(`Sanctum primary instance is configured: ${envConfig.SANCTUM_PRIMARY_INSTANCE_URL}`);

    await server.register(forwardWritesToPrimary, { primaryUrl: envConfig.SANCTUM_PRIMARY_INSTANCE_URL as string });
  }
