...
import { SANCTUM_PROVIDER_GITHUB_ACCESS_TOKEN } from "@app/lib/config/const";
          ...
          try {
            ...
            if (appCfg.OTEL_TELEMETRY_COLLECTION_ENABLED) {
              authAttemptCounter.add(1, {
                "sanctum.user.email": email,
                "sanctum.user.id": loginResult.user.id,
                "sanctum.organization.id": loginResult.orgId,
                "sanctum.organization.name": loginResult.orgName,
                "sanctum.auth.method": AuthAttemptAuthMethod.GOOGLE,
                "sanctum.auth.result": AuthAttemptAuthResult.SUCCESS,
            ...
            if (appCfg.OTEL_TELEMETRY_COLLECTION_ENABLED) {
              authAttemptCounter.add(1, {
                "sanctum.user.email": email,
                "sanctum.auth.method": AuthAttemptAuthMethod.GOOGLE,
                "sanctum.auth.result": AuthAttemptAuthResult.FAILURE,
                "client.address": requestContext.get(RequestContextKey.Ip),
                "user_agent.original": requestContext.get(RequestContextKey.UserAgent)
              });
            }
          ...
          try {
            ...
            if (appCfg.OTEL_TELEMETRY_COLLECTION_ENABLED) {
              authAttemptCounter.add(1, {
                ...
                "sanctum.organization.id": loginResult.orgId,
                "sanctum.organization.name": loginResult.orgName,
                "sanctum.auth.method": AuthAttemptAuthMethod.GITHUB,
                "sanctum.auth.result": AuthAttemptAuthResult.SUCCESS,
                "client.address": requestContext.get(RequestContextKey.Ip),
                "user_agent.original": requestContext.get(RequestContextKey.UserAgent)
              });
          ...
          try {
            ...
            if (appCfg.OTEL_TELEMETRY_COLLECTION_ENABLED) {
              authAttemptCounter.add(1, {
                ...
                "sanctum.organization.id": loginResult.orgId,
                "sanctum.organization.name": loginResult.orgName,
                "sanctum.auth.method": AuthAttemptAuthMethod.GITLAB,
                "sanctum.auth.result": AuthAttemptAuthResult.SUCCESS,
                "client.address": requestContext.get(RequestContextKey.Ip),
                "user_agent.original": requestContext.get(RequestContextKey.UserAgent)
              });
...
export const registerSsoRouter = async (server: FastifyZodProvider) => {
  ...
  await server.register(fastifySession, {
    ...
    cookie: {
      ...
      sameSite: "lax" // we want cookies to be sent to Sanctum in redirects originating from IDP server
    }
  });
  ...
  server.route({
    ...
    handler: async (req: any, res: any) => {
      if (req.passportUser.externalProviderAccessToken) {
        void res.cookie(SANCTUM_PROVIDER_GITHUB_ACCESS_TOKEN, req.passportUser.externalProviderAccessToken, {
          httpOnly: true,
          path: "/api",
          sameSite: "strict",
          secure: appCfg.HTTPS_ENABLED,
