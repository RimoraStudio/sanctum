...
export const registerServeUI = async (
  ...
  server.route({
    ...
    handler: (_req, res) => {
      ...
      const config = {
        ...
        CDN_HOST: cdnHost,
        PLATFORM_NAME: appCfg.PLATFORM_NAME
      };
      ...
      const js = `
        window.__SANCTUM_RUNTIME_ENV__ = Object.freeze(${JSON.stringify(config)});
        window.__toCdnUrl = function(filename) {
          var cdnHost = window.__SANCTUM_RUNTIME_ENV__.CDN_HOST || "";
  ...
  if (standaloneMode) {
    ...
    let indexHtml = fs
      ...
      .replace("<title>Sanctum</title>", `<title>${appCfg.PLATFORM_NAME}</title>`);
    ...
    // Inject GTM/analytics CSP domains only for Sanctum Cloud deployments
    if (appCfg.isCloud) {
