...
// If you rename the import, update the Dockerfile.fips.standalone-sanctum file as well.
...
const run = async () => {
  ...
  logger.info(
    `Running Sanctum ${envConfig.SANCTUM_PLATFORM_VERSION ? `v${envConfig.SANCTUM_PLATFORM_VERSION}` : "Development Mode"}`
  );
