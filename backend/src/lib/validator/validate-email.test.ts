...
describe("normalizeEmail", () => {
  test("strips sub-addressing on every provider", () => {
    expect(normalizeEmail("dave+cu24vk@sanctum.example")).toBe("dave@sanctum.example");
  ...
  // real recipients were third parties with no Sanctum account and do not belong in the repo.
  ...
  test.each([
    ...
    [
      "dave@sanctum.example",
      [
        "dave+cu24vk@sanctum.example",
        "dave+sf7yq@sanctum.example",
        "dave+kwyr@sanctum.example",
        "dave+qe0u@sanctum.example",
        "dave+dt5pd@sanctum.example"
      ]
    ]
...
describe("isAliasedEmail", () => {
  test("flags variants but not the canonical address", () => {
    expect(isAliasedEmail("dave+kwyr@sanctum.example")).toBe(true);
    expect(isAliasedEmail("first.last@gmail.com")).toBe(true);
    expect(isAliasedEmail("dave@sanctum.example")).toBe(false);
