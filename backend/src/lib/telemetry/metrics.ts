...
export const resolveCoreMeter = (): Meter => resolveMeter("SanctumCore");
...
const sanctumCoreMeter = lazyMeter("SanctumCore");
...
export const authAttemptCounter = sanctumMeter.createCounter("sanctum.auth.attempt.count", {
  description: "Authentication attempts (both successful and failed)",
  unit: "{attempt}"
});

export const secretReadCounter = sanctumMeter.createCounter("sanctum.secret.read.count", {
...
export const recordSecretReadMetric = (params: { environment: string; secretPath: string; name?: string }) => {
  if (shouldRecordHighCardinalityMetrics()) {
    const attributes: Record<string, string> = {
      "sanctum.environment": params.environment,
      "sanctum.secret.path": params.secretPath,
      ...(params.name ? { "sanctum.secret.name": params.name } : {})
    };
...
export const kmipOperationCounter = sanctumMeter.createCounter("sanctum.kmip.operation.count", {
...
export const recordKmipOperationMetric = (params: {
  ...
  if (shouldRecordHighCardinalityMetrics()) {
    const attributes: Record<string, string> = {
      "sanctum.kmip.operation.type": params.operationType,
      "sanctum.organization.id": params.orgId,
      "sanctum.project.id": params.projectId,
      "sanctum.kmip.client.id": params.clientId
    };

    if (params.objectId) {
      attributes["sanctum.kmip.object.id"] = params.objectId;
    }

    if (params.objectName) {
      attributes["sanctum.kmip.object.name"] = params.objectName;
    }
...
// -- New low-cardinality instruments (SanctumCore meter) ------------------------------------------

// Queue worker lifecycle metrics. Wired in queue-service.ts via worker.on('completed' | 'failed' | 'stalled').
export const queueJobCounter = sanctumCoreMeter.createCounter("sanctum.queue.job.count", {
  description: "Queue jobs processed by outcome (completed or failed)",
  unit: "{job}"
});

export const queueJobDurationHistogram = sanctumCoreMeter.createHistogram("sanctum.queue.job.duration", {
  description:
...
export const queueJobWaitHistogram = sanctumCoreMeter.createHistogram("sanctum.queue.job.wait", {
  description:
    "Queue job wait time (queue contention only). Subtracts job.opts.delay so intentional scheduling doesn't inflate percentiles.",
  unit: "s"
});

export const queueJobFailureCounter = sanctumCoreMeter.createCounter("sanctum.queue.job.failure.count", {
  description:
    "Queue job failures classified by error type. attempts.exhausted=true means all retries are spent (real failure).",
  unit: "{failure}"
});

export const queueStalledCounter = sanctumCoreMeter.createCounter("sanctum.queue.stalled.count", {
  description:
...
export const auditLogEnqueuedCounter = sanctumCoreMeter.createCounter("sanctum.audit_log.enqueued.count", {
  description: "Audit log events appended to the ingest stream for persistence, by event type and actor type.",
  unit: "{event}"
});

export const auditLogPersistDurationHistogram = sanctumCoreMeter.createHistogram(
  "sanctum.audit_log.persist.duration",
  {
...
export const auditLogDroppedCounter = sanctumCoreMeter.createCounter("sanctum.audit_log.dropped.count", {
  description:
...
export const auditLogStreamDeliveryDurationHistogram = sanctumCoreMeter.createHistogram(
  "sanctum.audit_log_stream.delivery.duration",
  {
...
export const auditLogStreamDeliveryExhaustedCounter = sanctumCoreMeter.createCounter(
  "sanctum.audit_log_stream.delivery.exhausted.count",
  {
    description:
...
export const permissionCacheLookupCounter = sanctumCoreMeter.createCounter(
  "sanctum.permission_cache.lookup.count",
  {
...
export const permissionCacheFingerprintDurationHistogram = sanctumCoreMeter.createHistogram(
  "sanctum.permission_cache.fingerprint.duration",
  {
...
export const secretCacheAccessCounter = sanctumCoreMeter.createCounter("sanctum.secret.cache.access.count", {
  description: "Secret cache accesses, labeled by result, whether If-None-Match was sent, and why it missed the 304.",
  unit: "{access}"
});

export const secretCacheEntryBytesHistogram = sanctumCoreMeter.createHistogram("sanctum.secret.cache.entry.bytes", {
  description: "Encrypted secret cache entry size computed at write time",
  unit: "By"
});

export const secretCacheOversizeSkipCounter = sanctumCoreMeter.createCounter(
  "sanctum.secret.cache.oversize_skip.count",
  {
...
// -- safeRequest HTTPS agent pool (SanctumCore meter) ----------------------------------------
...
export const safeRequestAgentEvictionCounter = sanctumCoreMeter.createCounter(
  "sanctum.safe_request.agent_eviction.count",
  {
    description:
...
export const coreHttpErrorCounter = sanctumCoreMeter.createCounter("sanctum.core.http.error.count", {
  description: "API errors with bounded error classification. Labels limited to SanctumCore View allowlist.",
  unit: "{error}"
});

// -- Signup abuse (SanctumCore meter) -------------------------------------------------------------
...
export const emailDispatchRequestCounter = sanctumCoreMeter.createCounter("sanctum.email_dispatch.request.count", {
  description:
...
export const emailDispatchDistinctCounter = sanctumCoreMeter.createCounter(
  "sanctum.email_dispatch.distinct.count",
  {
    description:
...
export const rateLimitExceededCounter = sanctumCoreMeter.createCounter("sanctum.rate_limit.exceeded.count", {
  description: "HTTP 429 responses (rate limit exceeded).",
  unit: "{request}"
});

// -- Authentication latency (SanctumCore meter) ---------------------------------------------------
export const authAttemptDurationHistogram = sanctumCoreMeter.createHistogram("sanctum.auth.attempt.duration", {
  description:
...
export const recordAuthAttemptMetric = (params: {
  ...
  if (!isTelemetryEnabled()) return;
  const attributes: Record<string, string> = {
    "sanctum.auth.method": params.method,
    "sanctum.auth.result": params.result
  };
...
export const tokenRenewalCounter = sanctumCoreMeter.createCounter("sanctum.auth.token.renewal.count", {
...
export const recordTokenRenewalMetric = (params: {
  ...
  if (params.authMethod) attributes["sanctum.auth.method"] = params.authMethod;
  if (params.error !== undefined) attributes["error.type"] = classifyError(params.error);
  tokenRenewalCounter.add(1, attributes);
};

// -- SCIM provisioning (SanctumCore meter) --------------------------------------------------------
...
export const scimOperationDurationHistogram = sanctumCoreMeter.createHistogram("sanctum.scim.operation.duration", {
...
// -- SSO config lifecycle (SanctumCore meter) -----------------------------------------------------
...
export const ssoConfigChangeCounter = sanctumCoreMeter.createCounter("sanctum.sso.config.change.count", {
...
// -- Secret operation metrics (SanctumCore meter) ------------------------------------------------
export const secretOperationDurationHistogram = sanctumCoreMeter.createHistogram(
  "sanctum.secret.operation.duration",
  {
...
export const secretWriteCounter = sanctumCoreMeter.createCounter("sanctum.secret.write.count", {
...
// -- Secret sync outcome (SanctumCore meter) ----------------------------------------------------
export const secretSyncOutcomeCounter = sanctumCoreMeter.createCounter("sanctum.secret_sync.outcome.count", {
  description:
...
// -- Secret rotation outcome (SanctumCore meter) --------------------------------------------------
export const secretRotationOutcomeCounter = sanctumCoreMeter.createCounter(
  "sanctum.secret_rotation.outcome.count",
  {
    description:
...
// -- Dynamic secret orphaned lease (SanctumCore meter) ---------------------------------------------
export const dynamicSecretOrphanedLeaseCounter = sanctumCoreMeter.createCounter(
  "sanctum.dynamic_secret.orphaned_lease.count",
  {
...
export const alertDispatchOutcomeCounter = sanctumCoreMeter.createCounter("sanctum.alert.dispatch.outcome.count", {
  description:
...
export const productAnalyticsPublishedCounter = sanctumCoreMeter.createCounter(
  "sanctum.product_analytics.published.count",
  {
...
export const productAnalyticsDroppedCounter = sanctumCoreMeter.createCounter(
  "sanctum.product_analytics.dropped.count",
  {
    description:
...
export const productAnalyticsBacklogHistogram = sanctumCoreMeter.createHistogram(
  "sanctum.product_analytics.shard.backlog",
  {
    description:
...
// -- Boot-time observable gauges (SanctumCore meter) ----------------------------------------------
...
export const registerInfrastructureMetrics = (db: Knex) => {
  ...
  const buildInfoGauge = meter.createObservableGauge("sanctum.build.info", {
    description: "Always 1. Labels carry build identification (version, git sha, node version)."
  });
  buildInfoGauge.addCallback((result) => {
    if (!isTelemetryEnabled()) return;
    result.observe(1, {
      "service.version": process.env.SANCTUM_PLATFORM_VERSION || "unknown",
  ...
  const dbPoolGauge = meter.createObservableGauge("sanctum.db.pool.connections", {
  ...
  // Read alongside sanctum.safe_request.agent_eviction.count — size pinned at max with a
  // non-zero eviction rate is the signal that the cap is too low.
  const agentPoolGauge = meter.createObservableGauge("sanctum.safe_request.agent_pool.size", {
...
// -- Legacy root-key usage (SanctumCore meter) -----------------------------------------------------
...
export const legacyRootKeyUsageCounter = sanctumCoreMeter.createCounter("sanctum.legacy_root_key.usage", {
  description:
