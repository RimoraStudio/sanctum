// Allowlist for the "SanctumCore" meter. Any attribute not listed is dropped by the SDK View.
...
export const SANCTUM_CORE_METER_ATTRIBUTES = [
  ...
  "db.pool.state",
  "pool.max",
  // Closed enums on sanctum.legacy_root_key.usage; per-project attribution is in logs, not here.
...
// Every meter that predates the SanctumCore allowlist. None have a View, so their per-actor / unbounded
...
export const HIGH_CARDINALITY_METER_NAMES = ["Sanctum", "API", "SecretSyncs", "PkiSyncs", "Integrations"];
