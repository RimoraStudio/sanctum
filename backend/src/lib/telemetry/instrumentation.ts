...
import { HIGH_CARDINALITY_METER_NAMES, SANCTUM_CORE_METER_ATTRIBUTES } from "./telemetry-attributes";
...
const initTelemetryInstrumentation = ({
  ...
  const serviceVersion = process.env.SANCTUM_PLATFORM_VERSION || "unknown";

  const resource = defaultResource().merge(
    resourceFromAttributes({
      [ATTR_SERVICE_NAME]: "sanctum-core",
  ...
  const views: ViewOptions[] = [
    {
      meterName: "SanctumCore",
      attributesProcessors: [createAllowListAttributesProcessor(SANCTUM_CORE_METER_ATTRIBUTES)]
    }
  ];
