...
// @infisical/quic ships no Windows prebuilt binding — load lazily so importing
// this module (e.g. via knexfile or boot) doesn't crash on platforms without it.
let quicCache: typeof import("@infisical/quic") | null = null;
const getQuic = async () => {
  if (!quicCache) {
    const mod = await import("@infisical/quic");
    quicCache = (mod.default || mod) as typeof import("@infisical/quic");
  }
  return quicCache;
};
...
const createQuicConnection = async (
  ...
  const client = await quic.QUICClient.createQUICClient({
    ...
    config: {
      ...
      key: tlsOptions.key,
      applicationProtos: ["sanctum-gateway"],
      verifyPeer: true,
