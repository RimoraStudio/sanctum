...
const GITHUB_LATEST_RELEASE_URL = "https://api.github.com/repos/Sanctum/sanctum/releases/latest";
...
export const updateCheckServiceFactory = ({ cronJob, keyStore }: TUpdateCheckServiceFactoryDep) => {
  ...
  const isEnabled = isUpdateCheckEnabled({
    isSanctumCloud: appCfg.SANCTUM_CLOUD,
    ...
    platformVersion: appCfg.SANCTUM_PLATFORM_VERSION
  });
  ...
  const getAvailableUpdateVersion = async () => {
    ...
    const current = parseSemanticVersion(appCfg.SANCTUM_PLATFORM_VERSION);
