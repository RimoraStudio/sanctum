...
export const userServiceFactory = ({
  ...
  const sendEmailVerificationCode = async (token: string) => {
    ...
    await smtpService.sendMail({
      ...
      subjectLine: `Sanctum confirmation code: ${userToken}`,
  ...
  const requestEmailChangeOTP = async ({ userId, newEmail }: TUpdateUserEmailDTO) => {
    ...
    const changeEmailOTP = await userDAL.transaction(async (tx) => {
      ...
      await smtpService.sendMail({
        ...
        subjectLine: "Confirm your Sanctum email change",
        recipients: [user.email],
        substitutions: {
    ...
    const result = await userDAL.transaction(async (tx) => {
      ...
      if (existingUser) {
        await smtpService.sendMail({
          ...
          subjectLine: "Email Change Request for Your Sanctum Account",
          recipients: [newEmail],
          substitutions: {
            email: newEmail
          }
        ...
        await smtpService.sendMail({
          template: SmtpTemplates.EmailVerification,
          subjectLine: "Sanctum email change verification",
          recipients: [newEmail],
          substitutions: {
    ...
    try {
      if (user?.email) {
        ...
        await smtpService.sendMail({
          ...
          subjectLine: "Your Sanctum account has been deleted",
          recipients: [user.email],
          substitutions: {
            email: user.email
          }
