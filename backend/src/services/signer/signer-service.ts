...
type TResolvedSanctumReissue = {
  isHsm: false;
  switchingKeySource: boolean;
  keyAlgorithm: CertKeyAlgorithm;
};
type TResolvedReissueTarget = TResolvedHsmReissue | TResolvedSanctumReissue;
...
export const signerServiceFactory = ({
  ...
  const create = async (dto: TCreateSignerDTO) => {
    ...
    if (
      ...
      throw new BadRequestError({
        message:
          "HSM-backed signers are not supported with AD CS yet. Use AWS Private CA, an Internal CA, or switch the signer's key source to Sanctum-managed."
      });
    }
  ...
  const reissueCertificate = async (dto: TReissueCertificateDTO) => {
    ...
    if ((reissueCaType === CaType.AZURE_AD_CS || reissueCaType === CaType.ADCS) && target.isHsm) {
      throw new BadRequestError({
        message:
          "HSM-backed signers are not supported with AD CS yet. Use AWS Private CA, an Internal CA, or switch the signer's key source to Sanctum."
      });
    }
