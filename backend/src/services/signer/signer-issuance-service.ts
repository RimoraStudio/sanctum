...
export const signerIssuanceServiceFactory = ({
  ...
  const stepSyncWithCsr = async (
    ...
    if (kind === CaType.AZURE_AD_CS) {
      if (job.keySource === CertKeySource.Hsm) {
        throw new BadRequestError({
          message:
            "HSM-backed signers are not supported with Azure AD CS yet. Use AWS Private CA, or switch the signer's key source to Sanctum."
        });
      }
      ...
      if (job.keySource === CertKeySource.Hsm) {
        throw new BadRequestError({
          message:
            "HSM-backed signers are not supported with Active Directory Certificate Service yet. Use AWS Private CA, or switch the signer's key source to Sanctum."
        });
      }
  ...
  const stepSyncWithDigiCert = async (job: TPkiSignerCertificateIssuanceJobs) => {
    ...
    if (!existingOrderId) {
      const csr = (await decryptForProject(projectId, Buffer.from(job.encryptedCsr as Buffer))).toString();
      const comments = `Issued via Sanctum signer ${job.signerId}`;
      ...
      const placementRef = `sanctum-signer-job-${job.id}`;
