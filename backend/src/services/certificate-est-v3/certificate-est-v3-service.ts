...
export const certificateEstV3ServiceFactory = ({
  ...
  const simpleEnrollByProfile = async ({
    ...
    if (result.status === CertificateRequestStatus.PENDING_APPROVAL) {
      throw new BadRequestError({
        message: `Certificate request requires approval. Certificate Request ID: ${result.certificateRequestId}. Manage the approval request in the Sanctum UI.`
      });
    }
  ...
  const simpleReenrollByProfile = async ({
    ...
    if (result.status === CertificateRequestStatus.PENDING_APPROVAL) {
      throw new BadRequestError({
        message: `Certificate re-enrollment request requires approval. Certificate Request ID: ${result.certificateRequestId}. Manage the approval request in the Sanctum UI.`
      });
    }
