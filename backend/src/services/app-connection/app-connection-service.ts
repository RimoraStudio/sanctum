...
import { ValidateExternalSanctumConnectionCredentialsSchema } from "./external-sanctum";
import { externalSanctumConnectionService } from "./external-sanctum/external-sanctum-connection-service";
...
const VALIDATE_APP_CONNECTION_CREDENTIALS_MAP: Record<AppConnection, TValidateAppConnectionCredentialsSchema> = {
  ...
  [AppConnection.ExternalSanctum]: ValidateExternalSanctumConnectionCredentialsSchema,
...
export const appConnectionServiceFactory = ({
    ...
    if (gatewayId) {
      ...
      if (!plan.gateway) {
        throw new BadRequestError({
          message:
            "Your current plan does not support gateway usage with app connections. Please upgrade your plan or contact Sanctum Sales for assistance."
        });
      }
  ...
  return {
    ...
    externalSanctum: externalSanctumConnectionService(connectAppConnectionById),
