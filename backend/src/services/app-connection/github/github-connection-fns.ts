...
export const assertPlatformGitHubHostAllowed = (host?: string | null) => {
  ...
  if (isCloud && normalizedHost !== PLATFORM_GITHUB_CREDENTIAL_HOST) {
    throw new BadRequestError({
      message:
        "GitHub App and OAuth connections to a custom host (e.g. GitHub Enterprise Server) are only supported on self-hosted Sanctum, where you can register your own GitHub App. To use a custom host, self-host Sanctum — or use the Personal Access Token method on Sanctum Cloud."
    });
  }
};
