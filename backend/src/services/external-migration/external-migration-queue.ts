...
import { importDataIntoSanctumFn } from "./external-migration-fns";
import { ExternalPlatforms, TImportSanctumDataCreate } from "./external-migration-types";
...
export const externalMigrationQueueFactory = ({
  ...
  queueService.start(QueueName.ImportSecretsFromExternalSource, async (job) => {
    ...
    try {
      await notificationService.createUserNotifications([
        {
          ...
          body: `An import from **${importType}** to Sanctum has been started.`
        }
      ]);

      await smtpService.sendMail({
        recipients: [actorEmail],
        subjectLine: "Sanctum import started",
        substitutions: {
          provider: importType
        },
      ...
      const decryptedJson = JSON.parse(decrypted) as TImportSanctumDataCreate;

      const { projectsNotImported } = await importDataIntoSanctumFn({
        input: decryptedJson,
        projectDAL,
        projectEnvDAL,
        secretDAL,
      ...
      await notificationService.createUserNotifications([
        {
          ...
          body: `An import from **${importType}** to Sanctum has successfully completed.`
        }
      ]);

      await smtpService.sendMail({
        recipients: [actorEmail],
        subjectLine: "Sanctum import successful",
        substitutions: {
          provider: importType
        },
      ...
      await notificationService.createUserNotifications([
        {
          ...
          body: `An import from **${importType}** to Sanctum has failed: ${(err as any)?.message || "Unknown error"}.`
        }
      ]);

      await smtpService.sendMail({
        recipients: [job.data.actorEmail],
        subjectLine: "Sanctum import failed",
        substitutions: {
          provider: importType,
