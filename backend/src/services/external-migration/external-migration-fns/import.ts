...
import { TImportDataIntoSanctumDTO } from "./envkey";

export const importDataIntoSanctumFn = async ({
  projectService,
  projectEnvDAL,
  projectDAL,
  secretDAL,
...
}: TImportDataIntoSanctumDTO) => {
  // Import data to sanctum
    ...
    if (data.secrets && data.secrets.length > 0) {
      ...
      for (const secret of data.secrets) {
        ...
        if (!originalToNewEnvironmentId.get(secret.environmentId) && !originalToNewFolderId.get(targetId)) {
          logger.info({ secret }, "[importDataIntoSanctumFn]: Could not find environment or folder for secret");
      ...
      for await (const [targetId, secrets] of mappedToEnvironmentId) {
        logger.info("[importDataIntoSanctumFn]: Processing secrets for targetId", targetId);
        ...
        if (foundFolder) {
          logger.info("[importDataIntoSanctumFn]: Processing secrets for folder");
          selectedFolder = await folderDAL.findById(foundFolder.folderId, tx);
          selectedProjectId = foundFolder.projectId;
        } else {
          logger.info("[importDataIntoSanctumFn]: Processing secrets for normal environment");
          ...
          if (!environment) {
            logger.info(
              ...
              "[importDataIntoSanctumFn]: Could not find environment for secret"
            );
            // eslint-disable-next-line no-continue
            continue;
          }
