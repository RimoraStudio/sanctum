...
import { SanctumImportData, KvVersion, VaultMappingType } from "../external-migration-types";
...
export const transformToSanctumFormatNamespaceToProjects = (
  vaultData: VaultData[],
...
export const transformToSanctumFormatKeyVaultToProjectsCustomC1 = (vaultData: VaultData[]): SanctumImportData => {
  const projects: Array<{ name: string; id: string }> = [];
  const environments: Array<{ name: string; id: string; projectId: string; envParentId?: string }> = [];
  const folders: Array<{ id: string; name: string; environmentId: string; parentFolderId?: string }> = [];
  const secrets: Array<{ id: string; name: string; environmentId: string; value: string; folderId?: string }> = [];
...
export const vaultMigrationTransformMappings: Record<
  string,
  (vaultData: VaultData[], mappingType: VaultMappingType) => SanctumImportData
> = {
  "68c57ab3-cea5-41fc-ae38-e156b10c14d2": transformToSanctumFormatKeyVaultToProjectsCustomC1
} as const;
...
export const importVaultDataFn = async (
  ...
  let transformFn: (vaultData: VaultData[], mappingType: VaultMappingType) => SanctumImportData;
  ...
  if (mappingType === VaultMappingType.Custom) {
    ...
    transformFn = transformToSanctumFormatNamespaceToProjects;
  }
