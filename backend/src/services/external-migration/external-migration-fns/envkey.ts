...
import { SanctumImportData, TEnvKeyExportJSON, TImportSanctumDataCreate } from "../external-migration-types";

export type TImportDataIntoSanctumDTO = {
  ...
  input: TImportSanctumDataCreate;
};
...
export const parseEnvKeyDataFn = async (decryptedJson: string): Promise<SanctumImportData> => {
  const parsedJson: TEnvKeyExportJSON = JSON.parse(decryptedJson) as TEnvKeyExportJSON;

  const sanctumImportData: SanctumImportData = {
    projects: [],
    environments: [],
    folders: [],
    secrets: []
  ...
  parsedJson.apps.forEach((app: { name: string; id: string }) => {
    sanctumImportData.projects.push({ name: app.name, id: app.id });
  });
  ...
  for (const env of parsedJson.baseEnvironments) {
    ...
    if (appId) {
      sanctumImportData.environments.push({
        id: env.id,
  ...
  const processBranches = () => {
    for (const subEnv of parsedJson.subEnvironments) {
      ...
      if (app) {
        ...
        const branchEnvironment = sanctumImportData.environments.find((e) => e.id === subEnv.parentEnvironmentId);

        // check if the folder already exists in the same parent environment with the same name

        const folderExists = sanctumImportData.folders.some(
        ...
        if (!folderExists) {
          sanctumImportData.folders.push({
            name: subEnv.subName,
        ...
        for (const { appId, orderIndex } of appsUsingBlock) {
          ...
          for (const [secretName, secretData] of Object.entries(branchSecrets)) {
            if (secretData.inheritsEnvironmentId) {
              ...
              const preExistingSecretIndex = sanctumImportData.secrets.findIndex(
                (s) => s.name === secretName && s.environmentId === matchingAppEnv.id
              );

              if (preExistingSecretIndex !== -1) {
                const preExistingSecret = sanctumImportData.secrets[preExistingSecretIndex];
                ...
                if (
                  ...
                  sanctumImportData.secrets[preExistingSecretIndex] = {
                    ...preExistingSecret,
              ...
              sanctumImportData.secrets.push({
  ...
  processBranches();
  processBlocksForApp(sanctumImportData.projects.map((app) => app.id));
  ...
  for (const env of Object.keys(parsedJson.envs)) {
    ...
    for (const [secretName, secretData] of Object.entries(envData.variables)) {
      const indexOfExistingSecret = sanctumImportData.secrets.findIndex(
        (s) =>
          s.name === secretName &&
      ...
      if (secretData.inheritsEnvironmentId) {
        ...
        if (indexOfExistingSecret !== -1) {
          ...
          const newSecret: (typeof sanctumImportData.secrets)[number] = {
            ...sanctumImportData.secrets[indexOfExistingSecret],
            value: resolvedSecret.val || ""
          };

          sanctumImportData.secrets[indexOfExistingSecret] = newSecret;
