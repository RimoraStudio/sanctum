...
export const kmsServiceFactory = ({
  ...
  /*
   * Generate KMS Key
   * This function is responsibile for generating the sanctum internal KMS for various entities
  ...
  const rotateKmsKey = async (kmsKeyId: string, tx?: Knex) => {
    ...
    const dbQuery = async (db: Knex) => {
      ...
      if (kmsDoc.externalKms) {
        throw new BadRequestError({
          message: "Cannot rotate external KMS keys from Sanctum. Rotate the key in your external provider instead."
        });
      }

      if (kmsDoc.isReserved) {
        throw new BadRequestError({ message: "Reserved Sanctum-managed KMS keys cannot be rotated." });
      }
  ...
  /*
   * Simple encryption service function to do all the encryption tasks in sanctum
  ...
  /*
   * Simple decryption service function to do all the encryption tasks in sanctum
   * This can be even later exposed directly as api for encryption as function
   */
  ...
  const decryptWithKmsKey = async ({
    ...
    if (kmsDoc.externalKms) {
      ...
      // This could be external kms(in future) but at the end of the day, the end KMS will be an sanctum internal one
