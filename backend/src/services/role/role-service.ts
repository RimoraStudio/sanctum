...
export const roleServiceFactory = ({
  ...
  const createRole = async (dto: TCreateRoleDTO) => {
    ...
    if (!plan?.rbac) {
      throw new BadRequestError({
        message:
          "Failed to create custom role due to plan RBAC restriction. Upgrade to Sanctum Enterprise plan to create custom roles."
      });
    }
  ...
  const updateRole = async (dto: TUpdateRoleDTO) => {
    ...
    if (!plan?.rbac) {
      throw new BadRequestError({
        message:
          "Failed to update custom role due to plan RBAC restriction. Upgrade to Sanctum Enterprise plan to update custom roles."
      });
    }
