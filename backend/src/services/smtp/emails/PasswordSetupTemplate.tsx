...
export const PasswordSetupTemplate = ({ email, isCloud, siteUrl, callback_url, token }: PasswordSetupTemplateProps) => {
  return (
    <BaseEmailWrapper title="Password Setup" preview="Setup your password for Sanctum." siteUrl={siteUrl}>
      <Heading className="text-black text-[18px] leading-[28px] text-center font-normal p-0 mx-0">
        <strong>Password Setup</strong>
      </Heading>
      <Section className="px-[24px] mb-[28px] mt-[36px] pt-[12px] pb-[8px] border border-solid border-gray-200 rounded-md bg-gray-50">
        <Text className="text-[14px]">Someone requested to set up a password for your Sanctum account.</Text>
        <Text className="text-[14px] text-red-600">
          Make sure you are already logged in to Sanctum in the current browser before clicking the link below.
        </Text>
        <Text className="text-[14px]">
          If you did not initiate this request, please contact{" "}
          {isCloud ? (
            <>
              us immediately at <BaseLink href="mailto:support@sanctum.com">support@sanctum.com</BaseLink>
            </>
          ) : (
...
PasswordSetupTemplate.PreviewProps = {
  email: "casey@sanctum.com",
  callback_url: "https://app.sanctum.com",
  isCloud: true,
