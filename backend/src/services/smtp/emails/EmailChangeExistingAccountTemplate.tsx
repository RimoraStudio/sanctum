...
export const EmailChangeExistingAccountTemplate = ({
  ...
  return (
    <BaseEmailWrapper
      ...
      preview="An email change was requested for an address that already has an Sanctum account."
      siteUrl={siteUrl}
    >
      ...
      <Section className="px-[24px] mt-[36px] pt-[12px] pb-[8px] text-[14px] border border-solid border-gray-200 rounded-md bg-gray-50">
        <Text className="text-[14px]">
          A request was made to change the email address of an Sanctum account to <strong>{email}</strong>.
        </Text>
        <Text className="text-[14px]">
          Because this address already belongs to an existing Sanctum account, the change was not completed. No
      ...
      <Section className="mt-[24px] bg-gray-50 pt-[2px] pb-[16px] border border-solid border-gray-200 px-[24px] rounded-md text-gray-800">
        <Text className="mb-[0px]">
          ...
          {isCloud ? (
            <>
              Contact us at <BaseLink href="mailto:support@sanctum.com">support@sanctum.com</BaseLink>
            </>
          ) : (
