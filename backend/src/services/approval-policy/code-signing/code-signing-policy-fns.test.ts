...
describe("commandsMatch", () => {
  const SIGNTOOL =
    '"C:\\Program Files (x86)\\Windows Kits\\10\\bin\\signtool.exe" sign /fd SHA256 /f cert.cer /csp "Sanctum KSP" /kc demo app.exe';
  ...
  test("quoting is part of the command", () => {
    expect(commandsMatch('tool --csp "Sanctum KSP"', 'tool --csp "Sanctum KSP"')).toBe(true);
    expect(commandsMatch('tool --csp "Sanctum KSP"', "tool --csp Sanctum KSP")).toBe(false);
  });
