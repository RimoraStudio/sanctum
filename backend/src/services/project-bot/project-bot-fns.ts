...
export const getBotKeyFnFactory = (
  ...
  const getBotKeyFn = async (projectId: string, shouldGetBotKey?: boolean) => {
    ...
    if (!bot || !bot.isActive || !bot.encryptedProjectKey || !bot.encryptedProjectKeyNonce) {
      ...
      if (!projectV1Keys) {
        throw new NotFoundError({
          message: `Project bot not found for project with ID '${projectId}'. Please ask an administrator to log-in to the Sanctum Console.`
        });
      }

      if (!projectV1Keys.senderPublicKey) {
        throw new NotFoundError({
          message: `Project bot not found for project with ID '${projectId}'. Please ask an administrator to log-in to the Sanctum Console and upgrade the project.`
        });
      }
