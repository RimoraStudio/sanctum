...
const syncSecretsAWSParameterStore = async ({
  ...
  if (awsAssumeRoleArn) {
    ...
    const command = new AssumeRoleCommand({
      ...
      RoleSessionName: `sanctum-parameter-store-${crypto.nativeCrypto.randomUUID()}`,
  ...
  if (metadata.kmsKeyId) {
    ...
    try {
      ...
      if (error instanceof AccessDeniedException) {
        logger.error(
          `AWS Parameter Store Error [integration=${integration.id}]: double check AWS account permissions (refer to the Sanctum docs)`
        );
      }
...
const syncSecretsAWSSecretManager = async ({
  ...
  if (awsAssumeRoleArn) {
    ...
    const command = new AssumeRoleCommand({
      RoleArn: awsAssumeRoleArn,
      RoleSessionName: `sanctum-sm-${crypto.nativeCrypto.randomUUID()}`,
      DurationSeconds: 900, // 15mins
      ExternalId: projectId
    });
...
const syncSecretsVercel = async ({
  ...
  const secretsToAddToSanctum: { [key: string]: VercelSecret } = {};
  ...
  Object.keys(res).forEach((vercelKey) => {
    if (!integration.lastUsed) {
      ...
      switch (metadata.initialSyncBehavior) {
        ...
        case IntegrationInitialSyncBehavior.OVERWRITE_TARGET: {
          if (!(vercelKey in sanctumSecrets)) sanctumSecrets[vercelKey] = null;
          break;
        }
        case IntegrationInitialSyncBehavior.PREFER_SOURCE: {
          // if the vercel secret is not in sanctum, we need to add it to sanctum
          if (!(vercelKey in sanctumSecrets)) {
            sanctumSecrets[vercelKey] = {
              value: res[vercelKey].value
            };
            secretsToAddToSanctum[vercelKey] = res[vercelKey];
          }
          break;
        }
        default: {
    ...
    } else if (!(vercelKey in sanctumSecrets)) {
      sanctumSecrets[vercelKey] = null;
    }
  });

  if (Object.keys(secretsToAddToSanctum).length) {
    await createManySecretsRawFn({
      projectId: integration.projectId,
      environment: integration.environment.slug,
      path: integration.secretPath,
      secrets: Object.keys(secretsToAddToSanctum).map((key) => ({
        secretName: key,
        secretValue: secretsToAddToSanctum[key].value,
        type: SecretType.Shared,
        secretComment: ""
      }))
    });
  ...
  for await (const key of Object.keys(sanctumSecrets)) {
    if (!(key in res) || sanctumSecrets[key]?.value !== res[key].value) {
      ...
      if (!(key in res)) {
        await request.post(
          ...
          {
            key,
            value: sanctumSecrets[key]?.value,
            type: "encrypted",
            ...(isCustomEnvironment
              ? {
...
const syncSecretsNetlify = async ({
  ...
  Object.keys(secrets).forEach((key) => {
    if (!(key in res)) {
      // case: Sanctum secret does not exist in Netlify -> create secret
      newSecrets.push({
        key,
        values: [
          {
      ...
      // case: Sanctum secret exists in Netlify
      ...
      if ((integration.targetEnvironment as string) in contexts) {
        ...
        if (secrets[key].value !== contexts[integration.targetEnvironment as string].value) {
          // case: Sanctum and Netlify secret values are different
  ...
  Object.keys(res).forEach((key) => {
    ...
    if (!(key in secrets)) {
      // case: Netlify secret does not exist in Sanctum
...
const syncSecretsGitLab = async ({
  ...
  if (!integration.lastUsed) {
    const secretsToAddToSanctum: { [key: string]: GitLabSecret } = {};
    ...
    getSecretsRes.forEach((gitlabSecret) => {
      ...
      switch (metadata.initialSyncBehavior) {
        ...
        case IntegrationInitialSyncBehavior.PREFER_SOURCE: {
          // if the secret is not in sanctum, we need to add it to sanctum
          if (!(gitlabSecret.key in secrets)) {
            secrets[gitlabSecret.key] = {
              value: gitlabSecret.value
            };
            // need to remove prefix and suffix from what we're saving to Sanctum
            ...
            secretsToAddToSanctum[processedKey] = gitlabSecret;
          }
          break;
        }
        default: {
...
const syncSecretsRundeck = async ({
  ...
  try {
    ...
    throw new Error(
      `Ensure that the provided Rundeck URL is accessible by Sanctum and that the linked API token has sufficient permissions.\n\n${
