...
export const RailwaySyncFns = {
   ...
   *
   * Upserts the Sanctum-managed secrets with `replace: false` so existing Railway variables are
  ...
  async syncSecrets(secretSync: TRailwaySyncWithCredentials, secretMap: TSecretMap) {
    try {
      ...
      // When deletion is enabled (full-mirror), remove only the variables Sanctum no longer
  ...
  async removeSecrets(secretSync: TRailwaySyncWithCredentials, secretMap: TSecretMap) {
    ...
    try {
      ...
      // Delete only the Sanctum-managed variables that exist in Railway. Per-key deletes (instead
