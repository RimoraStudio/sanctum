...
export const RenderSyncFns = {
  syncSecrets: async (secretSync: TRenderSyncWithCredentials, secretMap: TSecretMap) => {
    ...
    for (const renderSecret of renderSecrets) {
      const existsInSanctum = Boolean(secretMap[renderSecret.key]);
      const isManagedByThisSync = matchesSchema(renderSecret.key, environmentSlug, keySchema);

      if (existsInSanctum) {
        // Secret exists in both Render and Sanctum - will be updated in Step 2
        // eslint-disable-next-line no-continue
        continue;
      }

      // Secret exists in Render but NOT in Sanctum
    ...
    // Step 2: Process all secrets from Sanctum (these will overwrite any duplicates from Step 1)
    ...
    // Step 4: Delete secrets that were removed from Sanctum, only for environment groups because it doesn't delete on the update call
