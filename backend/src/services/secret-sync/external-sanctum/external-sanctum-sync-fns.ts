...
import { getExternalSanctumAccessToken } from "@app/services/app-connection/external-sanctum";
import { SecretSyncError } from "@app/services/secret-sync/secret-sync-errors";
import { matchesSchema } from "@app/services/secret-sync/secret-sync-fns";
import { TSecretMap } from "@app/services/secret-sync/secret-sync-types";

import { TExternalSanctumSyncWithCredentials } from "./external-sanctum-sync-types";
...
const withExternalSanctumErrorHandling = async <T>(fn: () => Promise<T>): Promise<T> => {
  try {
    return await fn();
  } catch (err) {
...
const getRemoteContext = async (secretSync: TExternalSanctumSyncWithCredentials): Promise<TRemoteContext> => {
  const { credentials } = secretSync.connection;
  const accessToken = await getExternalSanctumAccessToken(credentials);
...
const fetchRemoteSecrets = async (
  secretSync: TExternalSanctumSyncWithCredentials,
...
export const ExternalSanctumSyncFns = {
  syncSecrets: async (secretSync: TExternalSanctumSyncWithCredentials, secretMap: TSecretMap) =>
    withExternalSanctumErrorHandling(async () => {
  ...
  getSecrets: async (secretSync: TExternalSanctumSyncWithCredentials): Promise<TSecretMap> =>
    withExternalSanctumErrorHandling(async () => {
      const ctx = await getRemoteContext(secretSync);
      const remoteSecrets = await fetchRemoteSecrets(secretSync, ctx);
  ...
  removeSecrets: async (secretSync: TExternalSanctumSyncWithCredentials, secretMap: TSecretMap) =>
    withExternalSanctumErrorHandling(async () => {
      const ctx = await getRemoteContext(secretSync);
