...
export const azureKeyVaultSyncFactory = ({
  ...
  const syncSecrets = async (secretSync: TAzureKeyVaultSyncWithCredentials, secretMap: TSecretMap) => {
    ...
    Object.keys(secretMap).forEach((sanctumKey) => {
      const hyphenatedKey = sanctumKey.replaceAll("_", "-");
      if (!(hyphenatedKey in vaultSecrets)) {
        // case: secret has been created
        setSecrets.push({
          key: hyphenatedKey,
          value: secretMap[sanctumKey].value
        });
      } else if (secretMap[sanctumKey].value !== vaultSecrets[hyphenatedKey].value) {
