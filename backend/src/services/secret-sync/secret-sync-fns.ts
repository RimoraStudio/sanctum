...
import { EXTERNAL_SANCTUM_SYNC_LIST_OPTION, ExternalSanctumSyncFns } from "./external-sanctum";
...
const SECRET_SYNC_LIST_OPTIONS: Record<SecretSync, TSecretSyncListItem> = {
  ...
  [SecretSync.ExternalSanctum]: EXTERNAL_SANCTUM_SYNC_LIST_OPTION,
...
export const SecretSyncFns = {
  syncSecrets: (
    ...
    switch (secretSync.destination) {
      ...
      case SecretSync.ExternalSanctum:
        // Key schema is intentionally not applied for Sanctum-to-Sanctum syncs to prevent
        // infinite sync loops where the prefixed key triggers another sync cycle.
        return ExternalSanctumSyncFns.syncSecrets(secretSync, secretMap);
      case SecretSync.OVH:
  ...
  getSecrets: async (
    ...
    switch (secretSync.destination) {
      ...
      case SecretSync.AzureEntraIdScim:
        ...
        break;
      case SecretSync.ExternalSanctum:
        secretMap = await ExternalSanctumSyncFns.getSecrets(secretSync);
        break;
      case SecretSync.OVH:
  ...
  removeSecrets: (
    ...
    switch (secretSync.destination) {
      ...
      case SecretSync.ExternalSanctum:
        // Key schema is intentionally not applied for Sanctum-to-Sanctum syncs to prevent
        // infinite sync loops where the prefixed key triggers another sync cycle.
        return ExternalSanctumSyncFns.removeSecrets(secretSync, secretMap);
      case SecretSync.OVH:
