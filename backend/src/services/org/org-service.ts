...
export const orgServiceFactory = ({
  ...
  const addGhostUser = async (orgId: string, tx?: Knex) => {
    const email = `sudo-${alphaNumericNanoId(16)}-${orgId}@sanctum.com`; // We add a nanoid because the email is unique. And we have to create a new ghost user each time, so we can have access to the private key.
    ...
    await Promise.all(
      resolvedInvites.map(async ({ invitedUser, org }) => {
        ...
        try {
          await smtpService.sendMail({
            template: SmtpTemplates.OrgInvite,
            subjectLine: `Reminder: You have been invited to ${org.name} on Sanctum`,
