...
export const certificateRenewalServiceFactory = ({
    ...
    if (isCsrAuthoritative) {
      ...
      if (originalCert.altNames && requestedAltNames.length === 0) {
        throw new BadRequestError({
          message:
            "Every subject alternative name cannot be removed while reusing this certificate's key pair, because Sanctum does not hold its private key and can only renew from the original signing request. Supply a CSR with the names you want, or renew with a new key pair."
        });
      }
  ...
  const $loadRenewalSubject = async (
    ...
    if (keySource === CertificateRenewalKeySource.Reuse && !certificateSecret && !originalCsr) {
      throw new BadRequestError({
        message:
          "This certificate's key pair cannot be reused because Sanctum holds neither its private key nor the signing request it was issued from. Renew with a new key pair, or for a CA-issued certificate supply a CSR generated from the existing key."
      });
    }
  ...
  const $assertKeySourceSupported = ({
    ...
    if (issuerType !== IssuerType.CA) {
      ...
      if (!hasStoredKey) {
        throw new BadRequestError({
          message:
            "This self-signed certificate's key pair cannot be reused because Sanctum does not hold its private key, which is what signs a self-signed certificate. Renew with a new key pair."
        });
      }
      return;
    }
