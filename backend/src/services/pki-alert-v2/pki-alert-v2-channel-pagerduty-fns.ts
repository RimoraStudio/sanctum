...
export const buildPagerDutyPayload = ({
  ...
  const summary =
    eventType === PkiAlertEventType.EXPIRATION
      ? `Sanctum: ${totalCertificates} certificate(s) expiring within ${alert.alertBefore} - Alert: ${alert.name}`
      : `Sanctum: ${totalCertificates} certificate(s) ${summaryVerb} - Alert: ${alert.name}`;
  ...
  return {
    ...
    links: [{ href: viewUrl, text: "View certificates in Sanctum" }]
  };
};
