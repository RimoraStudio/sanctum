...
import { SANCTUM_SECRET_VALUE_HIDDEN_MASK } from "../secret/secret-fns";
    ...
    processedBatchImports.forEach(({ importPath, importEnv, id, folderId, isReserved }, i) => {
      ...
      const secretsWithDuplicate = (importedSecretsGroupByFolderId?.[importedFolders?.[i]?.id as string] || [])
        ...
        .map((item) => ({
          ...
          secretValue: viewSecretValue ? activeDecryptor(item.encryptedValue) : SANCTUM_SECRET_VALUE_HIDDEN_MASK,
