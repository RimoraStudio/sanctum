...
describe("NetScaler buildManagedCertNamePattern (orphan cleanup detection)", () => {
  describe("default pattern (no schema configured)", () => {
    ...
    test("matches default Sanctum names and their renewal suffixes", () => {
      expect(pattern.test(`Sanctum-${HEX}`)).toBe(true);
      expect(pattern.test(`Sanctum-${HEX}-1`)).toBe(true);
    });

    // Deletion safety: never treat a non-Sanctum certkey as managed.
    test("does NOT match unrelated certkeys", () => {
      ...
      expect(pattern.test(`Sanctum-${HEX}.bad`)).toBe(false);
    });
  });
