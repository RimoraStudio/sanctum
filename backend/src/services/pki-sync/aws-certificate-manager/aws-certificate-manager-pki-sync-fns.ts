...
const SANCTUM_CERTIFICATE_TAG = "SanctumCertificate";
...
const generateCertificateName = (certificateName: string, pkiSync: TPkiSyncWithCredentials): string => {
  ...
  if (certificateNameSchema) {
    ...
    if (sanitizedCertificateName.startsWith("Sanctum-")) {
      certificateId = sanitizedCertificateName.substring("Sanctum-".length);
    } else {
    ...
    if (canRemoveCertificates) {
      ...
      Object.values(acmCertificates).forEach((acmCert) => {
        if (acmCert.arn && acmCert.Tags) {
          const hasSanctumTag = acmCert.Tags.some((tag) => tag.Key === SANCTUM_CERTIFICATE_TAG && tag.Value);
