...
const parseOptions = (overrides: Record<string, unknown>) =>
  KempLoadMasterPkiSyncOptionsSchema.safeParse({ certificateNameSchema: "Sanctum-{{certificateId}}", ...overrides });
...
describe("Kemp LoadMaster caCertificateNameSchema", () => {
  ...
  test("applies sensible defaults when omitted", () => {
    ...
    if (result.success) {
      expect(result.data.caCertificateNameSchema).toBe("Sanctum-ca-{{fingerprint}}");
    }
  });

  test("does not force any placeholder (user names CAs freely, like the certificate schema)", () => {
    expect(acceptsCa("Sanctum-ca-{{fingerprint}}")).toBe(true);
    expect(acceptsCa("Sanctum-ca-{{commonName}}")).toBe(true);
