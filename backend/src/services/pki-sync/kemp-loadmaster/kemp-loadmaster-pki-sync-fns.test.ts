...
describe("Kemp buildManagedCertNamePattern (orphan cleanup detection)", () => {
  test("default pattern matches Sanctum names and renewal suffixes, not unrelated certs", () => {
    const pattern = buildManagedCertNamePattern(undefined);
    expect(pattern.test(`Sanctum-${HEX}`)).toBe(true);
    expect(pattern.test(`Sanctum-${HEX}-1`)).toBe(true);
  ...
  test("schema-based pattern anchors on the resolved placeholders", () => {
    const pattern = buildManagedCertNamePattern("Sanctum-{{certificateId}}");
    expect(pattern.test(`Sanctum-${HEX}`)).toBe(true);
    expect(pattern.test("Sanctum-notavalidid")).toBe(false);
  });
});

describe("Kemp buildIntermediateIdentifier", () => {
  test("compiles {{fingerprint}} to a stable, content-addressed 24-char hex", () => {
    const a = buildIntermediateIdentifier(TEST_CA_PEM, "Sanctum-ca-{{fingerprint}}");
    const b = buildIntermediateIdentifier(TEST_CA_PEM, "Sanctum-ca-{{fingerprint}}");
    expect(a).toBe(b);
    expect(new RE2("^Sanctum-ca-[0-9a-f]{24}$").test(a)).toBe(true);
    expect(buildIntermediateIdentifier("different-content", "Sanctum-ca-{{fingerprint}}")).not.toBe(a);
  });
