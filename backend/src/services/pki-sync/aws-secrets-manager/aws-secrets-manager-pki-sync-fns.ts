...
const isSanctumManagedCertificate = (secretName: string, pkiSync: TPkiSyncWithCredentials): boolean => {
  ...
  return secretName.startsWith(AWS_SECRETS_MANAGER_PKI_SYNC_DEFAULTS.SANCTUM_PREFIX);
};
      ...
      try {
        ...
        if (output.SecretList) {
          output.SecretList.forEach((secretEntry) => {
            if (
              secretEntry.Name &&
              isSanctumManagedCertificate(secretEntry.Name, pkiSync as unknown as TPkiSyncWithCredentials)
            ) {
  ...
  const syncCertificates = async (
    ...
    for (const [certName, certData] of Object.entries(certificateMap)) {
      ...
      if (!syncOptions?.certificateNameSchema) {
        targetSecretName = `${AWS_SECRETS_MANAGER_PKI_SYNC_DEFAULTS.SANCTUM_PREFIX}${certificateId}`;
      }
            ...
            () =>
              client.send(
                new CreateSecretCommand({
                  ...
                  SecretString: secretValue,
                  KmsKeyId: keyId,
                  Description: `Certificate managed by Sanctum`
                })
              ),
            {
