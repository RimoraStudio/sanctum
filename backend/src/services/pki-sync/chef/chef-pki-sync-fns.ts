...
const isSanctumManagedCertificate = (certificateName: string, pkiSync: TPkiSyncWithCredentials): boolean => {
  ...
  return certificateName.startsWith(CHEF_PKI_SYNC_DEFAULTS.SANCTUM_PREFIX);
};
  ...
  const syncCertificates = async (
    ...
    if (canRemoveCertificates) {
      ...
      Object.keys(chefDataBagItems).forEach((itemName) => {
        ...
        if (isTracked || (allowPatternCleanup && isSanctumManagedCertificate(itemName, pkiSync))) {
