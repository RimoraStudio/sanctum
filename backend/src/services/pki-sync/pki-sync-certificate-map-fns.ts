      ...
      try {
        ...
        if (certBody) {
          ...
          if (certificateNameSchema) {
            ...
            certificateName = `Sanctum-${stableId}`;
          }

          const alternativeNames: string[] = [];

          const legacyName = `Sanctum-${certificate.id.replace(DASH_REGEX, "")}`;
          if (legacyName !== certificateName) {
            alternativeNames.push(legacyName);
          }

          if (cert.renewedFromCertificateId) {
            const originalLegacyName = `Sanctum-${cert.renewedFromCertificateId.replace(DASH_REGEX, "")}`;
