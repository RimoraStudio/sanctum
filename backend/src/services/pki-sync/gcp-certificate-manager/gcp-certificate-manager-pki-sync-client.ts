...
export const mapGcpError = (
  ...
  if (status === GcpErrorStatus.ResourceExhausted || httpStatus === 429 || httpStatus === 503) {
    return new PkiSyncError({
      message: `GCP Certificate Manager rate limited the ${context.operation} request. Sanctum will retry.`,
      context
    });
  }
...
export const createGcpCertificateManagerClient = ({
  ...
  const upsertCertificate = async ({
    ...
    const body = {
      description: "Managed by Sanctum",
      labels,
    ...
    if (shouldPatch) {
      try {
        ...
        // The certificate was deleted outside Sanctum; fall through and recreate it.
