...
const assertScopeCompatibility = (
  ...
  if (config.certificateMapBinding) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["certificateMapBinding"],
      message: `Certificate map binding is only available for global certificates, but this sync targets "${config.location}". Regional Application Load Balancers attach certificates directly to the target HTTPS proxy, which Sanctum does not manage.`
    });
  }
};
...
const GcpLabelSchema = z.object({
  key: z
    ...
    .refine((value) => !(GCP_RESERVED_LABEL_KEYS as readonly string[]).includes(value), {
      message: `The label keys ${GCP_RESERVED_LABEL_KEYS.join(" and ")} are set by Sanctum and cannot be overridden`
    }),
  value: z
    .string()
    .trim()
...
export const GcpCertificateManagerPkiSyncOptionsSchema = BasePkiSyncOptionsSchema.omit({
  ...
  certificateNameSchema: buildDestinationCertificateNameSchema({
    ...
    message:
      'Certificate name schema must start with a lowercase letter, because GCP Certificate Manager requires a resource ID to start with a letter and a placeholder can resolve to a digit. Prefix the schema, for example "sanctum-{{certificateId}}".'
  })
});
