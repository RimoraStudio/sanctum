...
type TUploadTarget = {
  ...
  sanctumCertificateId?: string;
  renewedFromCertificateId?: string;
  renewedFromResourceName?: string;
  shouldPatch: boolean;
};
...
const buildLabels = (userLabels: TGcpCertificateManagerPkiSyncOptions["labels"], sanctumCertificateId?: string) => ({
  ...Object.fromEntries((userLabels ?? []).map(({ key, value }) => [key, value])),
  [GCP_MANAGED_BY_LABEL_KEY]: GCP_MANAGED_BY_LABEL_VALUE,
  ...(sanctumCertificateId ? { [GCP_CERTIFICATE_ID_LABEL_KEY]: sanctumCertificateId.replace(DASH_REGEX, "") } : {})
});

const isSanctumManaged = (labels?: Record<string, string>) =>
...
export const gcpCertificateManagerPkiSyncFactory = ({
  ...
  const $reconcileCertificateMapEntry = async ({
    ...
    const desiredResourceNames = Array.from(
      new Set([
        ...certificateResourceNames,
        ...retainResourceNames.filter((name) => existingEntry?.certificates?.includes(name))
      ])
    );

    if (existingEntry && !isSanctumManaged(existingEntry.labels)) {
      throw new PkiSyncError({
        shouldRetry: false,
        message: `Certificate map entry "${entryId}" already exists in map "${certificateMap}" and is not managed by Sanctum. Remove or rename it before enabling certificate map binding.`
      });
    }
    ...
    try {
      if (!existingEntry) {
        await client.createCertificateMapEntry({
          certificateMap,
          entryId,
          certificateResourceNames: desiredResourceNames,
          hostname,
      ...
      if (sameCertificateSet(existingEntry.certificates, desiredResourceNames)) return;
  ...
  const $detachAndDeleteCertificate = async ({
    ...
    if (certificateMapBinding) {
      ...
      try {
        ...
        if (entry && isSanctumManaged(entry.labels) && entry.certificates?.includes(resourceName)) {
  ...
  const $reapOrphanedMapEntries = async ({
    ...
    certificates.forEach((certificate) => {
      if (!isSanctumManaged(certificate.labels)) return;
  ...
  const $planCertificate = async ({
    ...
    if (existing && !isSanctumManaged(existing.labels)) {
      return {
        skipReason: `A certificate named "${gcpCertificateId}" already exists in GCP and is not managed by Sanctum. Rename the certificate name schema or remove the existing certificate.`
      };
    }
    ...
    return {
      target: {
        ...
        sanctumCertificateId: certificateId,
        renewedFromCertificateId: renewedFromCertificateId ?? undefined,
        renewedFromResourceName: renewedFromCertificateId
          ? (existingSyncRecord?.externalIdentifier ?? undefined)
          : undefined,
  ...
  const $planUploads = async ({
    ...
    const retainResourceNames = new Set<string>();
    ...
    for (const [certName, entry] of Object.entries(certificateMap)) {
      ...
      if ("skipReason" in planned) {
        ...
        const record = entry.certificateId ? syncRecordsByCertId.get(entry.certificateId) : undefined;
        if (record?.externalIdentifier) retainResourceNames.add(record.externalIdentifier);
      } else {
    ...
    return { uploadTargets, skippedCertificates, activeResourceNames, trackedCertificateIds, retainResourceNames };
  };

  const $servingResourceName = (target?: TUploadTarget) => {
    if (!target) return undefined;
    return target.shouldPatch ? target.resourceName : target.renewedFromResourceName;
  };
  ...
  const $uploadCertificate = async ({
    ...
    try {
      await client.upsertCertificate({
        ...
        labels: buildLabels(userLabels, target.sanctumCertificateId),
        scope,
    ...
    if (existingCertSync) {
      ...
      await certificateSyncDAL.addCertificates(pkiSync.id, [
        { certificateId: target.sanctumCertificateId, externalIdentifier: target.resourceName }
      ]);
    }

    if (target.renewedFromCertificateId && target.renewedFromResourceName === target.resourceName) {
    ...
    for (const candidate of candidates) {
      try {
        ...
        if (!candidate.keepSyncRecord) {
          await certificateSyncDAL.removeCertificates(pkiSync.id, [candidate.sanctumCertificateId]);
        }
        removed += 1;
      } catch (error) {
        ...
        await certificateSyncDAL.updateSyncStatus(
          pkiSync.id,
          candidate.sanctumCertificateId,
  ...
  const syncCertificates = async (
    ...
    const { uploadTargets, skippedCertificates, activeResourceNames, trackedCertificateIds, retainResourceNames } =
      await $planUploads({
        certificateMap,
        syncRecordsByCertId,
        existingCertificates,
    ...
    if (mapEntryOutOfSync) {
      logger.warn(
        { syncId: pkiSync.id, certificateMap: certificateMapBinding?.certificateMap },
    ...
    const { removed, failedRemovals } =
      canRemoveCertificates && !mapEntryOutOfSync
        ? await $removeStaleCertificates({
            ...
            uploadedCertificateIds: new Set(
              successfulUploads
                .map(({ value }) => value.sanctumCertificateId)
  ...
  const removeCertificates = async (
    ...
    const targets = certificateNames.flatMap((certName) => {
      const sanctumCertificateId = deps?.certificateMap?.[certName]?.certificateId;
      if (!sanctumCertificateId) return [];

      const record = existingSyncRecords.find((row) => row.certificateId === sanctumCertificateId);
    ...
    for (const target of targets) {
      try {
        ...
        await certificateSyncDAL.removeCertificates(pkiSync.id, [target.sanctumCertificateId]);
        removed += 1;
      } catch (error) {
        const message = error instanceof Error ? error.message : "Unknown error";
