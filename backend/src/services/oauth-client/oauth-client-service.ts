...
export const oauthClientServiceFactory = ({
  ...
  // Token exchange turns externally-issued tokens into Sanctum user tokens, so establishing that trust
  ...
  const checkSsoConfigPermission = async (actor: OrgServiceActor, action: string) => {
    ...
    if (permission.cannot(OrgPermissionSsoActions.Edit, OrgPermissionSubjects.Sso)) {
      throw new ForbiddenRequestError({
        message: `You do not have permission to ${action}. Applications using the '${OauthGrantType.TokenExchange}' grant turn tokens from your organization's identity provider into Sanctum user tokens, so managing them also requires permission to edit the organization's SSO configuration.`
      });
    }
  };
  ...
  const exchangeSubjectToken = async (
    ...
    if (!userAlias) {
      throw new UnauthorizedError({
        message:
          "The user this token identifies has not signed in to Sanctum through your organization's OIDC SSO yet. They need to complete a browser sign-in once before an application can act on their behalf."
      });
    }
    ...
    if (!user) {
      throw new UnauthorizedError({ message: "The user this token identifies no longer has an Sanctum account." });
    }

    if (!user.isAccepted) {
      throw new UnauthorizedError({
        message: "The user this token identifies has not completed Sanctum account setup."
      });
    }
    ...
    if (user.isLocked || (user.temporaryLockDateEnd && new Date() < user.temporaryLockDateEnd)) {
      throw new UnauthorizedError({
        message:
          "The Sanctum account for the user this token identifies is locked, so an application cannot act on their behalf. They can unlock it by resetting their password."
      });
    }
