...
export const superAdminServiceFactory = ({
      ...
      usersToEmail.map(async (user) => {
        ...
        if (user.isAccepted) {
          return smtpService.sendMail({
            ...
            subjectLine: "You've been added to an Sanctum organization",
        ...
        return smtpService.sendMail({
          ...
          subjectLine: "Sanctum organization invitation",
          recipients: [user.email],
          substitutions: {
            inviterFirstName: serverAdmin?.firstName,
            inviterUsername: serverAdmin?.email,
  ...
  const grantServerAdminAccessToUser = async (userId: string) => {
    if (!licenseService.onPremFeatures?.instanceUserManagement) {
      throw new BadRequestError({
        message: "Failed to grant server admin access to user due to plan restriction. Upgrade to Sanctum's Pro plan."
      });
    }
  ...
  const updateRootEncryptionStrategy = async (strategy: RootKeyEncryptionStrategy) => {
    if (!licenseService.onPremFeatures.hsm) {
      throw new BadRequestError({
        message: "Failed to update encryption strategy due to plan restriction. Upgrade to Sanctum's Enterprise plan."
      });
    }
      ...
      const data = await emailDomainDAL.create(
        {
          ...
          verificationRecordName: `_sanctum-verification.${domain.toLowerCase()}`,
          status: "verified",
          verifiedAt: new Date(),
