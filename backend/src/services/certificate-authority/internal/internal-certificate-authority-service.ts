...
export const internalCertificateAuthorityServiceFactory = ({
  ...
  const createCa = async ({
    ...
    const keySource = dto.keySource ?? CertKeySource.Sanctum;
      ...
      const caSecret =
        useHsm && hsmKey
          ...
          : await certificateAuthoritySecretDAL.create(
              {
                caId: ca.id,
                keySource: CertKeySource.Sanctum,
                encryptedPrivateKey
              },
              tx
            );
   ...
   * Note: private key and CSR are generated within Sanctum.
   */
   ...
   * Note: CSR is generated externally and submitted to Sanctum.
   */
