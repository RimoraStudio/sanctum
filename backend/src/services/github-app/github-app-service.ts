...
export const gitHubAppServiceFactory = ({
  ...
  // GitHub Enterprise Server) isn't directly reachable from the Sanctum backend.
  ...
  // is deleted — at that point the app's private key is gone from Sanctum, so any installation
  ...
  const initiateManifestCreation = async ({
    ...
    const manifest = {
      ...
      description: "Sanctum GitHub App Connection",
      public: false,
