...
describe("emailDispatchGuard", () => {
  ...
  describe("hashAddress", () => {
    test("keys the code on the literal address and the throttles on the mailbox", () => {
      ...
      const { emailHash, mailboxHash } = guard.hashAddress("dave+work@sanctum.example");

      expect(emailHash).toBe(hmac("dave+work@sanctum.example"));
      expect(mailboxHash).toBe(hmac("dave@sanctum.example"));
    });
  ...
  describe("checkMailboxCooldown", () => {
    ...
    test.each([
      ...
      ["DAVE@sanctum.example", "dave@sanctum.example"],
      ["Dave+reset@sanctum.Example", "dave@sanctum.example"]
    ...
    test("signup and recovery do not share a bucket", async () => {
      ...
      await guard.checkMailboxCooldown({ purpose: EmailDispatchPurpose.Signup, email: "dave@sanctum.example" });
      await guard.checkMailboxCooldown({
        purpose: EmailDispatchPurpose.AccountRecovery,
        email: "dave@sanctum.example"
      });
    ...
    test("reports remaining TTL when the cooldown is active", async () => {
      ...
      const err = await expectRejected(
        guard.checkMailboxCooldown({ purpose: EmailDispatchPurpose.Signup, email: "dave@sanctum.example" }),
        BadRequestError
      );
  ...
  describe("startMailboxCooldown", () => {
    test("writes the cooldown for the normalized mailbox", async () => {
      const { keyStore, guard } = setup();
      const mailboxHash = hmac("dave@sanctum.example");
    ...
    test("loses the race gracefully when another request armed it first", async () => {
      ...
      const err = await expectRejected(
        guard.startMailboxCooldown({
          ...
          mailboxHash: hmac("dave@sanctum.example")
        }),
        BadRequestError
      );
  ...
  describe("probeTraffic", () => {
    test("stores only HMACs, never the address or the IP", async () => {
      ...
      expect(members).not.toContain("dave@sanctum.example");
    });
  });
});
