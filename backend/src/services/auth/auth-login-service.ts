    ...
    try {
      ...
      if (appCfg.OTEL_TELEMETRY_COLLECTION_ENABLED) {
        authAttemptCounter.add(1, {
          ...
          "sanctum.user.id": user.id,
          "sanctum.auth.method": AuthAttemptAuthMethod.EMAIL,
    ...
    if (isRecoveryCode && userEnc.email) {
      await smtpService
        .sendMail({
          ...
          subjectLine: "A recovery code was used to sign in to your Sanctum account",
  ...
  const oauth2Login = async ({
    ...
    if (!user) {
      ...
      if (existingAlias && user.email !== sanitizedEmail && isTrustedForAccount) {
        ...
        if (conflictingUser && conflictingUser.id !== user.id) {
          throw new BadRequestError({
            message:
              "Unable to complete login: the email associated with your SSO account is already in use by another Sanctum user.",
            name: "Oauth 2 login"
          });
        }
    ...
    if (user.email && existingAlias && !existingAlias.isEmailVerified) {
      ...
      await smtpService.sendMail({
        ...
        subjectLine: `Sanctum confirmation code: ${verificationCode}`,
        recipients: [user.email],
        substitutions: {
        ...
        await smtpService
          .sendMail({
            ...
            substitutions: {
              ...
              ip: ipAddress,
              userAgent,
              siteUrl: removeTrailingSlash(cfg.SITE_URL || "https://app.sanctum.com"),
              orgId: organizationId
            },
