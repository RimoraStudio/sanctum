...
export const registerV1EERoutes = async (server: FastifyZodProvider) => {
  ...
  // depreciated in favour of sanctum workspace
  ...
  await server.register(
    registerRemovedProductTombstoneRouter(
      "The Sanctum SSH product has been removed. SSH host access is now available through Sanctum PAM (https://sanctum.com/docs/documentation/platform/pam/overview). If you are using 'sanctum ssh' CLI commands, upgrade to the latest CLI version."
    ),
    { prefix: "/ssh" }
  );
  await server.register(
    registerRemovedProductTombstoneRouter(
      "The Sanctum Agent Sentinel (MCP) product has been removed. See Agent Proxy for agent traffic management (https://sanctum.com/docs/documentation/platform/agent-proxy/overview)."
    ),
    { prefix: "/ai/mcp" }
  );
