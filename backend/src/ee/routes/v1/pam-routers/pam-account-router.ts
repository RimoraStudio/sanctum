...
CA_FILE="/etc/ssh/sanctum_ca.pub"
SSHD_CONFIG="/etc/ssh/sshd_config"

echo "==> Sanctum SSH CA Setup"
echo ""
...
if grep -qxF "\${CA_PUBLIC_KEY}" "\${CA_FILE}"; then
    ...
    echo "    Added Sanctum CA key to \${CA_FILE}"
fi

if [ "\${CONFIGURE_SSHD}" = "true" ]; then
    echo "==> Adding TrustedUserCAKeys to \${SSHD_CONFIG}..."
    echo "" >> "\${SSHD_CONFIG}"
    echo "# Sanctum SSH CA - Added by setup script" >> "\${SSHD_CONFIG}"
...
echo "==> Setup complete!"
echo ""
echo "Your SSH server is now configured to trust certificates signed by the Sanctum CA."
echo ""
`;
