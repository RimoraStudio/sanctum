...
import { isSanctumProjectTemplate } from "@app/ee/services/project-template/project-template-fns";
...
export const registerProjectTemplateRouter = async (server: FastifyZodProvider) => {
  server.route({
    ...
    handler: async (req) => {
      ...
      const auditTemplates = projectTemplates.filter((template) => !isSanctumProjectTemplate(template.name));
      ...
      body: z
        .object({
          name: slugSchema({ field: "name" })
            .refine((val) => !isSanctumProjectTemplate(val), {
