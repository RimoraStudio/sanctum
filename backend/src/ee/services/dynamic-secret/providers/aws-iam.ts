...
export const AwsIamProvider = (): TDynamicProviderFns => {
  ...
  const $getClient = async (providerInputs: z.infer<typeof DynamicSecretAwsIamSchema>, projectId: string) => {
    ...
    if (providerInputs.method === AwsIamAuthType.AssumeRole) {
      ...
      const command = new AssumeRoleCommand({
        ...
        RoleSessionName: `sanctum-dynamic-secret-${crypto.nativeCrypto.randomUUID()}`,
        ...
        if (providerInputs.method === AwsIamAuthType.AssumeRole) {
          ...
          await stsClient.send(
            new AssumeRoleCommand({
              RoleArn: providerInputs.roleArn,
              RoleSessionName: `sanctum-validation-${crypto.nativeCrypto.randomUUID()}`,
  ...
  const create = async (data: {
      ...
      try {
        ...
        if (providerInputs.method === AwsIamAuthType.AssumeRole) {
          ...
          const assumeRoleRes = await stsClient.send(
            new AssumeRoleCommand({
              RoleArn: providerInputs.roleArn,
              RoleSessionName: `sanctum-temp-cred-${crypto.nativeCrypto.randomUUID()}`,
    ...
    if (providerInputs.credentialType === AwsIamCredentialType.IamUser) {
      ...
      const awsTags = [{ Key: "createdBy", Value: "sanctum-dynamic-secret" }];
        ...
        if (policyDocument) {
          await client.send(
            new PutUserPolicyCommand({
              ...
              PolicyName: `sanctum-dynamic-policy-${alphaNumericNanoId(4)}`,
