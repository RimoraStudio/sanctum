...
const ElastiCacheUserManager = (credentials: TBasicAWSCredentials, region: string) => {
  ...
  const sanctumGroup = "sanctum-managed-group-elasticache";

  const ensureSanctumGroupExists = async (clusterName: string) => {
    const replicationGroups = await elastiCache.send(new DescribeUserGroupsCommand());

    const existingGroup = replicationGroups.UserGroups?.find((group) => group.UserGroupId === sanctumGroup);

    let newlyCreatedGroup = false;
    if (!existingGroup) {
      const createGroupCommand = new CreateUserGroupCommand({
        UserGroupId: sanctumGroup,
        UserIds: ["default"],
        Engine: "redis"
      });
    ...
    if (existingGroup || newlyCreatedGroup) {
      ...
      if (!replicationGroup?.UserGroupIds?.includes(sanctumGroup)) {
        // If the replication group doesn't have the sanctum user group, we need to associate it
        const modifyGroupCommand = new ModifyReplicationGroupCommand({
          UserGroupIdsToAdd: [sanctumGroup],
  ...
  const $addUserToSanctumGroup = async (userId: string) => {
  ...
  const createUser = async (creationInput: TCreateElastiCacheUserInput, clusterName: string) => {
    await ensureSanctumGroupExists(clusterName);

    await elastiCache.send(new CreateUserCommand(creationInput)); // First create the user
    await $addUserToSanctumGroup(creationInput.UserId); // Then add the user to the group. We know the group is already a part of the cluster because of ensureSanctumGroupExists()
