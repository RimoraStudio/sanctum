...
export const verifyHostInputValidity = async ({
  ...
  if (isAppUsedIps)
    throw new BadRequestError({
      message: `The host ${host} belongs to a service that is in-use by Sanctum, such as the Sanctum database or Redis instance. You cannot use hosts that are in-use by Sanctum.`
    });
  return inputHostIps;
};
