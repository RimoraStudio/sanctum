...
export const githubOrgSyncServiceFactory = ({
  ...
  const syncUserGroups = async (orgId: string, userId: string, accessToken: string) => {
    ...
    const sanctumUserGroups = await userGroupMembershipDAL.findGroupMembershipsByUserIdInOrg(userId, orgId);
    const sanctumUserGroupSet = new Set(sanctumUserGroups.map((el) => el.groupName));
    ...
    const data = await octokit.graphql
      ...
      .catch((err) => {
        if ((err as Error)?.message?.includes("Although you appear to have the correct authorization credential")) {
          throw new BadRequestError({
            message:
              "Please check your organization have approved Sanctum Oauth application. For more info: https://sanctum.com/docs/documentation/platform/github-org-sync#troubleshooting"
          });
        }
    ...
    const githubUserTeamOnSanctum = await groupDAL.find({ orgId, $in: { name: githubUserTeams } });
    const githubUserTeamOnSanctumGroupByName = groupBy(githubUserTeamOnSanctum, (i) => i.name);

    const newTeams = githubUserTeams.filter(
      (el) => !sanctumUserGroupSet.has(el) && !Object.hasOwn(githubUserTeamOnSanctumGroupByName, el)
    );
    const updateTeams = githubUserTeams.filter(
      (el) => !sanctumUserGroupSet.has(el) && Object.hasOwn(githubUserTeamOnSanctumGroupByName, el)
    );
    const removeFromTeams = sanctumUserGroups.filter((el) => !githubUserTeamSet.has(el.groupName));
    ...
    if (newTeams.length || updateTeams.length || removeFromTeams.length) {
      ...
      if (updateTeams.length) {
        await groupDAL.transaction(async (tx) => {
          await userGroupMembershipDAL.insertMany(
            updateTeams.map((el) => ({
              groupId: githubUserTeamOnSanctumGroupByName[el][0].id,
              userId
            })),
            tx
          );
  ...
  const syncAllTeams = async ({ orgPermission }: TSyncAllTeamsDTO): Promise<TSyncResult> => {
      ...
      return octokit.graphql
        ...
        .catch((err) => {
          ...
          if ((err as Error)?.message?.includes("Although you appear to have the correct authorization credential")) {
            throw new BadRequestError({
              message:
                "Organization has restricted OAuth app access. Please check that: 1) Your organization has approved the Sanctum OAuth application, 2) The token owner has sufficient organization permissions."
            });
          }
    ...
    const existingTeamsOnSanctum = await groupDAL.find({
      orgId: orgPermission.orgId,
      $in: { name: allGithubTeamNames }
    });
    const existingTeamsMap = groupBy(existingTeamsOnSanctum, (i) => i.name);
