...
export const subOrgServiceFactory = ({
  ...
  const createSubOrg = async ({ name, slug, permission }: TCreateSubOrgDTO) => {
    ...
    if (!orgLicensePlan.subOrganization) {
      throw new BadRequestError({
        message: "Sub-organization creation failed. Please upgrade your instance to Sanctum's Enterprise plan."
      });
    }
  ...
  const joinSubOrg = async ({ subOrgId, permission }: TJoinSubOrgDTO) => {
    ...
    if (!orgLicensePlan.subOrganization) {
      throw new BadRequestError({
        message:
          "Sub-organization feature is not available. Please upgrade your instance to Sanctum's Enterprise plan."
      });
    }
