    ...
    if (user.email && (!userAlias.isEmailVerified || !user.isAccepted)) {
      ...
      await smtpService
        .sendMail({
          ...
          subjectLine: `Sanctum confirmation code: ${token}`,
        ...
        .catch((err: Error) => {
          throw new OidcAuthError({
            message: `Error sending email confirmation code for user registration - contact the Sanctum instance admin. ${err.message}`
          });
        });
    }
          ...
          .then((loginResult) => {
            ...
            if (appCfg.OTEL_TELEMETRY_COLLECTION_ENABLED) {
              authAttemptCounter.add(1, {
                "sanctum.user.email": claims?.email?.toLowerCase(),
                "sanctum.user.id": loginResult.userId,
                "sanctum.organization.id": org.id,
                "sanctum.organization.name": org.name,
                "sanctum.auth.method": AuthAttemptAuthMethod.OIDC,
