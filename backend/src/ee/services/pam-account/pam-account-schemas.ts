...
export const ACCOUNT_TYPE_CONFIGS = {
  [PamAccountType.Postgres]: {
    ...
    ui: {
      ...
      authMethod: {
        ...
        options: [
          { label: "Password", value: PamPostgresAuthMethod.Password },
          {
            ...
            docsUrl: "https://sanctum.com/docs/documentation/platform/pam/accounts/postgresql#aws-iam-authentication"
          }
        ]
      },
      awsRegion: {
      ...
      roleArn: {
        label: "Role ARN",
        tooltip:
          "The IAM role Sanctum assumes to generate the authentication token. It needs rds-db:connect on this account's database user, and its trust policy must allow Sanctum using your Sanctum Organization ID as the External ID.\n\nOrganization ID: {{organizationId}}"
      },
      sslEnabled: {
        label: "SSL Enabled",
        forceWhen: [
  ...
  [PamAccountType.AwsIam]: {
    ...
    ui: {
      roleArn: {
        label: "Role ARN",
        tooltip:
          "The ARN of the IAM role Sanctum assumes when accessing this account. Its trust policy must allow Sanctum using your Sanctum Organization ID as the External ID.\n\nOrganization ID: {{organizationId}}"
      }
    }
  },
  ...
  [PamAccountType.AzureCli]: {
    ...
    ui: {
      ...
      clientId: {
        label: "Client ID",
        tooltip: "The application (client) ID of the service principal Sanctum brokers access to."
      },
      clientSecret: {
        label: "Client Secret",
