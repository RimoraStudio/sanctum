...
// Sanctum assumes the account's role directly (single hop). The role's trust policy must allow
// Sanctum's AWS account with the org's Sanctum ID as the External ID (confused-deputy guard).
const assumeRole = async ({
  ...
  if (!result.Credentials) {
    throw new BadRequestError({
      message:
        "Failed to assume role - verify the role's trust policy allows Sanctum using your Sanctum Organization ID as the External ID"
    });
  }
