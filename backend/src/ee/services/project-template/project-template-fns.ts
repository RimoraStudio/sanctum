...
export const getDefaultProjectTemplate = (orgId: string, type: ProjectType) => ({
  ...
  type,
  name: SanctumProjectTemplate.Default,
  createdAt: new Date(),
  updatedAt: new Date(),
  description: `Sanctum's ${type} default project template`,
...
export const isSanctumProjectTemplate = (template: string) =>
  Object.values(SanctumProjectTemplate).includes(template as SanctumProjectTemplate);
