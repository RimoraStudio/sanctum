...
export const projectTemplateServiceFactory = ({
  ...
  const createProjectTemplate: TProjectTemplateServiceFactory["createProjectTemplate"] = async (
    ...
    if (environments && plan.environmentLimit !== null && environments.length > plan.environmentLimit) {
      throw new BadRequestError({
        ...
        message: `Failed to create project template due to environment count exceeding your current limit of ${plan.environmentLimit}. Contact Sanctum to increase limit.`
      });
    }
  ...
  const updateProjectTemplateById: TProjectTemplateServiceFactory["updateProjectTemplateById"] = async (
    ...
    if (environments && plan.environmentLimit !== null && environments.length > plan.environmentLimit) {
      throw new BadRequestError({
        // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
        message: `Failed to update project template due to environment count exceeding your current limit of ${plan.environmentLimit}. Contact Sanctum to increase limit.`
      });
    }
