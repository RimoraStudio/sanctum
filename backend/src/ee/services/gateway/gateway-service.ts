...
export const gatewayServiceFactory = ({
  ...
  const $validateOrgAccessToGateway = async (orgId: string, actorId: string, actorAuthMethod: ActorAuthMethod) => {
    ...
    //     message:
    //       "Gateway handshake failed due to instance plan restrictions. Please upgrade your instance to Sanctum's Enterprise plan."
    //   });
    // }
    ...
    if (!orgLicensePlan.gateway) {
      throw new BadRequestError({
        message:
          "Gateway handshake failed due to organization plan restrictions. Please upgrade your instance to Sanctum's Enterprise plan."
      });
    }
  ...
  const getGatewayRelayDetails = async (actorId: string, actorOrgId: string, actorAuthMethod: ActorAuthMethod) => {
    ...
    return {
      ...
      sanctumStaticIp: envCfg.GATEWAY_SANCTUM_STATIC_IP_ADDRESS
    };
  };
  ...
  const exchangeAllocatedRelayAddress = async ({
    ...
    const orgGatewayConfig = await orgGatewayConfigDAL.transaction(async (tx) => {
      ...
      const rootCaCert = await x509.X509CertificateGenerator.createSelfSigned({
        name: `O=${identityOrg},CN=Sanctum Gateway Root CA`,
  ...
  const fnGetGatewayClientTlsByGatewayId = async (gatewayId: string): Promise<TGatewayV1RelayDetails> => {
    ...
    if (!orgLicensePlan.gateway) {
      throw new BadRequestError({
        message: "Please upgrade your instance to Sanctum's Enterprise plan to use gateways."
      });
    }
