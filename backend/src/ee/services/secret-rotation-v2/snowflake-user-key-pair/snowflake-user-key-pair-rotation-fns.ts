...
> = (secretRotation) => {
  ...
  const ensureUserExists = async () => {
    await runSnowflake(async (client) => {
      ...
      if (!resolvedUsername) {
        ...
        await executeSnowflakeSql(
          client,
          `CREATE USER ${quoteSnowflakeIdent(username.toUpperCase())} TYPE = SERVICE COMMENT = 'Created by Sanctum secret rotation'`
        );
      }
  ...
  > = async (credentialsToRevoke, callback) => {
    ...
    // Only unset slots holding a key Sanctum generated, so we never remove a key set outside this rotation.
