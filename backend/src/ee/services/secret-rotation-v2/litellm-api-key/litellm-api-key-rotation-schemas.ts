...
/** Max length for the user-provided key name. Sanctum appends a timestamp, so this is a sanity cap. */
...
const AdditionalOptionsSchema = z
  ...
  .superRefine((val, ctx) => {
    ...
    if (reservedKeys.length) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: `The following options are managed by Sanctum and cannot be set: ${reservedKeys.join(", ")}.`
      });
    }
  });
