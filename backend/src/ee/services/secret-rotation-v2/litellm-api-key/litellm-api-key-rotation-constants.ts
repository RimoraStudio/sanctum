 ...
 * Sanctum owns them for the rotation lifecycle or because they have dedicated form fields:
 * - key_alias is generated per key (embeds creation time + guarantees uniqueness)
 * - auto_rotate / rotation_interval would conflict with Sanctum's create/delete rotation
 * - duration would expire the key out from under Sanctum
...
/**
 * Additional options that must not be set on rotated keys. Unlike reserved options (Sanctum-owned
