...
// One shape for both routes to the API server: straight out from Sanctum, or tunnelled through a
  ...
  try {
    ...
    if (err instanceof BadRequestError) {
      throw new BadRequestError({
        message: `Kubernetes host '${kubernetesHost}' is not an allowed address: ${err.message}. Use an address Sanctum can reach over the public internet, or use Token or AWS auth for a private cluster.`
      });
    }
    throw err;
  }
...
export const validateKubernetesConfigReachable = async ({
  ...
  // Nothing to probe when Sanctum will fall back to letting the incoming token review itself:
