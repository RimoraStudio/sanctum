...
// Routes TokenReview traffic through a gateway instead of straight out from Sanctum, for clusters
...
//   kubernetesHost    -> TCP tunnel to that address, and Sanctum performs the review as usual.
    ...
    withGatewayV2Proxy(
      async (port) => {
        ...
        const config = {
          headers: {
            ...headers,
            ...(isGatewayReviewer ? { "x-sanctum-action": GatewayHttpProxyActions.UseGatewayK8sServiceAccount } : {})
          },
          httpsAgent,
