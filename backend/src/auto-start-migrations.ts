...
const migrationConfig = {
  ...
  tableName: "sanctum_migrations"
};

const getImageVersionTag = (): string => {
  const version = getConfig().SANCTUM_PLATFORM_VERSION;
  if (!version) {
    return "development";
  }
...
const logUnknownAppliedMigrations = ({
  ...
  if (dateRange) {
    sections.push(
      `The database contains migrations dated up to ${dateRange.latest} that this image does not bundle. If this is not a rolling deployment, verify the intended image version is deployed — an Sanctum image released on or after ${dateRange.latest} should include them.`
    );
  } else {
...
const throwInvalidMigrationHistory = ({
  ...
  const sections = [
    ...
    `This usually means the database was migrated by a different (typically newer) Sanctum image than the one currently running — for example after a rollback or a mixed-version rollout.`,
  ...
  if (unknownDateRange) {
    sections.push(
      `To resolve: deploy an Sanctum image released on or after ${unknownDateRange.latest} so its bundled migrations include the unknown ones above, or restore the database to a state matching this image's migrations.`
    );
  } else {
    sections.push(
      `To resolve: deploy a newer Sanctum image whose bundled migrations include the unknown ones above, or restore the database to a state matching this image's migrations.`
    );
  }
