...
describe.each([{ auth: AuthMode.JWT }, { auth: AuthMode.IDENTITY_ACCESS_TOKEN }])(
  ...
  async ({ auth }) => {
    ...
    test("Bulk upsert secrets in path multiple paths", async () => {
      ...
      // bulk ones should exist
      const firstBatchSecretsOnSanctum = await getSecrets(seedData1.environment.slug, secretTestCases[0].path);
      expect(firstBatchSecretsOnSanctum).toEqual(
        expect.arrayContaining(
      ...
      const secondBatchSecretsOnSanctum = await getSecrets(seedData1.environment.slug, secretTestCases[1].path);
      expect(secondBatchSecretsOnSanctum).toEqual(
        expect.arrayContaining(
