...
USAGE:
    docker run --rm sanctum/sanctum npm run --silent assets:export [-- OPTIONS] [PATH]
...
EXAMPLES:
    ...
    docker run --rm sanctum/sanctum npm run --silent assets:export > assets.tar

    # Extract to local directory
    docker run --rm -v $(pwd)/cdn-assets:/output sanctum/sanctum npm run --silent assets:export /output
