...
const IDP_ENTRY_POINT = "https://preview-orchestrator.sanctum.workers.dev/saml-idp/sso";
const IDP_ISSUER = "https://preview-orchestrator.sanctum.workers.dev/saml-idp";
...
// that might already exist for Sanctum's primary org.
const E2E_EMAIL_DOMAIN = "sanctum-e2e.test";
...
const guardAgainstProd = (host: string, dbName: string) => {
  if ((process.env.SANCTUM_ENV ?? "").toLowerCase() === "prod") {
    throw new Error("Refusing to run: SANCTUM_ENV=prod");
  }
  ...
  if (!existingDomain) {
    const [domainRow] = await knex(TableName.EmailDomains)
      .insert({
        ...
        verificationRecordName: `_sanctum-verify.${E2E_EMAIL_DOMAIN}`,
        status: "verified",
        verifiedAt: new Date(),
