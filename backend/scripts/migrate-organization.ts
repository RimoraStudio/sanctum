...
const exportDb = () => {
  ...
  const exportUser = sanitizeInputParam(
    prompt("Enter your Postgres User to migrate from: [Default = sanctum]: ") ?? "sanctum"
  );
  ...
  const exportDatabase = sanitizeInputParam(
    prompt("Enter your Postgres Database to migrate from [Default = sanctum]: ") ?? "sanctum"
  );

  // we do not include the audit_log and secret_sharing entries
  execSync(
    `PGDATABASE=${exportDatabase} PGPASSWORD=${exportPassword} PGHOST=${exportHost} PGPORT=${exportPort} PGUSER=${exportUser} pg_dump -Fc sanctum --exclude-table-data="secret_sharing" --exclude-table-data="audit_log*" > ${path.join(
      __dirname,
      "../src/db/backup.dump"
    )}`,
    { stdio: "inherit" }
...
const importDbForOrg = () => {
  ...
  const importUser = sanitizeInputParam(
    prompt("Enter your Postgres User to migrate to: [Default = sanctum]: ") ?? "sanctum"
  );
  ...
  const importDatabase = sanitizeInputParam(
    prompt("Enter your Postgres Database to migrate to [Default = sanctum]: ") ?? "sanctum"
  );
