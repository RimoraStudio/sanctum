...
BASE_URL = os.environ.get("SANCTUM_API_URL", "http://localhost:8080")
...
AUTH_TOKEN = os.environ.get("SANCTUM_TOKEN")
BOOTSTRAP_SANCTUM = int(os.environ.get("BOOTSTRAP_SANCTUM", 0))
TECHNITIUM_URL = os.environ.get("TECHNITIUM_URL", "http://localhost:5380")
TECHNITIUM_USER = os.environ.get("TECHNITIUM_USER", "admin")
TECHNITIUM_PASSWORD = os.environ.get("TECHNITIUM_PASSWORD", "sanctum")


# Called mostly from a CI to setup the new Sanctum instance to get it ready for BDD tests
def bootstrap_sanctum(context: Context):
    bootstrap_result_file = pathlib.Path.cwd() / ".bdd-sanctum-bootstrap-result.json"
    ...
    with httpx.Client(base_url=BASE_URL) as client:
        resp = client.post(
            "/api/v1/admin/signup",
            json={
                "email": f"{faker.user_name()}@sanctum.com",
...
def before_all(context: Context):
    ...
    if BOOTSTRAP_SANCTUM:
        details = bootstrap_sanctum(context)
        vars = base_vars | {
