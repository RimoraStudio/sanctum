Feature: Access Control
  ...
  Scenario Outline: Access resources across different account
    ...
    Then I register a new ACME account with email fangpen@sanctum.com and EAB key id "{acme_profile.eab_kid}" with secret "{acme_profile.eab_secret}" as acme_account0
    ...
    When I have an ACME client connecting to "{BASE_URL}/api/v1/cert-manager/acme/applications/{acme_profile.app_id}/profiles/{acme_profile.id}/directory"
    Then I register a new ACME account with email maidu@sanctum.com and EAB key id "{acme_profile.eab_kid}" with secret "{acme_profile.eab_secret}" as acme_account1
    Then I peak and memorize the next nonce as nonce
    When I send a raw ACME request to "<url>"
      """
      {
  ...
  Scenario Outline: Access resources across a different profiles
    ...
    Then I register a new ACME account with email maidu@sanctum.com and EAB key id "{acme_profile1.eab_kid}" with secret "{acme_profile1.eab_secret}" as acme_account1
    Then I peak and memorize the next nonce as nonce
    Then I memorize <src_var> with jq "<jq>" as <dest_var>
    When I send a raw ACME request to "<url>"
      """
