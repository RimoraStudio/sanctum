Feature: Account
  ...
  Scenario: Create a new account with the same key pair twice
    ...
    And I register a new ACME account with email fangpen@sanctum.com and EAB key id "{acme_profile.eab_kid}" with secret "{acme_profile.eab_secret}" as acme_account2
  ...
  Scenario: Find an existing account
    ...
    And I find the existing ACME account with email fangpen@sanctum.com and EAB key id "{acme_profile.eab_kid}" with secret "{acme_profile.eab_secret}" as retrieved_account
  ...
  Scenario: Create a new account without EAB
    Given I have an ACME cert profile as "acme_profile"
    When I have an ACME client connecting to "{BASE_URL}/api/v1/cert-manager/acme/applications/{acme_profile.app_id}/profiles/{acme_profile.id}/directory"
    Then I register a new ACME account with email fangpen@sanctum.com without EAB
  ...
  Scenario Outline: Scenario: Create a new account with bad EAB credentials
    Given I have an ACME cert profile as "acme_profile"
    When I have an ACME client connecting to "{BASE_URL}/api/v1/cert-manager/acme/applications/{acme_profile.app_id}/profiles/{acme_profile.id}/directory"
    Then I register a new ACME account with email fangpen@sanctum.com and EAB key id "<eab_kid>" with secret "<eab_secret>" as acme_account
  ...
  Scenario Outline: Scenario: Create a new account with bad EAB url
    ...
    Examples: Bad URLs
      | url                                                                                     |
      | {BASE_URL}/api/v1/cert-manager/acme/applications/{acme_profile.app_id}/profiles/{acme_profile.id}/new-account-bad          |
      | {BASE_URL}/api/v1/cert-manager/acme/applications/{acme_profile.app_id}/profiles/{acme_profile.id}/new-account?foo=bar      |
      | {BASE_URL}/api/v1/cert-manager/acme/applications/{acme_profile.app_id}/profiles/{acme_profile.id}/new-account#foobar       |
      | {BASE_URL}/acme/new-account                                                             |
      | https://example.com/api/v1/cert-manager/acme/applications/{acme_profile.app_id}/profiles/{acme_profile.id}/new-account-bad |
      | bad                                                                                     |
