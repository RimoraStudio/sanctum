Feature: Challenge
  ...
  Scenario: Did not finish all challenges
    ...
    # the sanctum.com auth should still be pending
    And I memorize order with jq ".authorizations | map(select(.body.identifier.value == "sanctum.com")) | first | .uri" as sanctum_auth
    And I post-as-get {sanctum_auth} as sanctum_auth_resp
    And the value sanctum_auth_resp with jq ".status" should be equal to "pending"
