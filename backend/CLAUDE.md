...
This is the **backend** package of the Sanctum monorepo — a Fastify 4 API server with TypeScript, PostgreSQL via Knex, and BullMQ queues.
...
The e2e suite is intended to run inside a FIPS image built
...
**`Dockerfile.fips-toolchain`**, published to
`ghcr.io/sanctum/backend-fips-toolchain` for amd64 and arm64. The CI workflows that built
and published these images were removed in the Sanctum fork; the notes below still apply to
local builds and to any CI that gets reinstated.

`Dockerfile.dev.fips` consumes it via the `TOOLCHAIN_IMAGE` build arg, so local dev
pulls the toolchain instead of spending ~15min compiling it. Consequences worth knowing:
...
- **The published image is an internal CI build cache, not a supported artifact.** It is
  ...
  guarantee. It is *not* the FIPS product image; that one is `sanctum/sanctum-fips`,
  built from `Dockerfile.fips.standalone-sanctum`. OCI labels on the image say the same.
- **Keep every third-party fetch pinned and checksummed.** SoftHSM2 is pinned to a commit
  ...
  SHA256-verified; the Sanctum CLI apt package is version-pinned. Oracle Instant Client
...
**Use `.env` rather than a one-off env var.** The usual start command is
`up --build`, which rebuilds the backend and
...
3. Add a corresponding alarm in the infrastructure repo so the new job is monitored. Follow the existing pattern in `sanctum-shared-cloud/modules/redis_alarms/main.tf` — every cron job must have an alarm defined there. Don't ship a new cron job without wiring up its alarm.
...
- **`SanctumCore`** — the meter for all new metrics. A strict attribute allowlist (`SANCTUM_CORE_METER_ATTRIBUTES` in `telemetry-attributes.ts`) is applied via an SDK View, so **only bounded labels survive** — HTTP method, parameterized `http.route` template, and low-cardinality enums. This is the single choke point: any attribute passed at a call site that isn't in the allowlist is silently dropped.
- **Legacy `Sanctum` / `API` / `SecretSyncs` / `PkiSyncs` / `Integrations`** — carry unbounded per-actor labels (`HIGH_CARDINALITY_METER_NAMES`). Disabled wholesale via `OTEL_DROP_HIGH_CARDINALITY_METERS=true` in multi-tenant/cloud.
...
**A new instrument goes on `SanctumCore` and carries only bounded attributes.** The per-actor labels on the legacy meters are a deliberate exception, not a precedent: they are allowed only through `highCardinalityMeter`, so that `OTEL_DROP_HIGH_CARDINALITY_METERS` can switch them off. Do not put unbounded attributes on any other meter, and do not acquire a meter outside `src/lib/telemetry/` — ESLint blocks both `@opentelemetry/*` imports and `getMeter` access there, so use `highCardinalityMeter` for per-actor instruments and `resolveCoreMeter` for `SanctumCore` observable gauges.
...
is never rotated; `sanctum.legacy_root_key.usage` is the evidence for when it can be deleted.
