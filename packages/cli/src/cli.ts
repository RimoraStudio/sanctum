...
const cmdAgents = (args: string[]) => {
  ...
  if (existing.includes("Secrets management (Sanctum)")) {
    const markerMatch = existing.match(/<!-- generated by sanctum-cli@(\S+)/);
    const sameVersion = markerMatch?.[1]?.replace(/\s*-.*$/, "") === VERSION;
    // blocks generated by a different CLI version are replaced without --force;
    // same-version blocks and markerless sections are left alone unless forced
    if (sameVersion && !force) {
      console.log(`AGENTS.md already has a Sanctum section from sanctum-cli@${VERSION}.`);
      return;
    }
    if (!markerMatch && !force) {
    ...
    // skip the Sanctum heading itself when locating the next section
