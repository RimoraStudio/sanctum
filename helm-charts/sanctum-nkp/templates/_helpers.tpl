...
app.kubernetes.io/name: {{ include "sanctum.name" . }}
...
*/}}
{{- define "sanctum.encryptionKey" -}}
{{- if .Values.secrets.encryptionKey -}}
{{- .Values.secrets.encryptionKey -}}
{{- else -}}
{{- $existing := lookup "v1" "Secret" .Release.Namespace (printf "%s-secrets" (include "sanctum.fullname" .)) -}}
...
*/}}
{{- define "sanctum.dbEnv" -}}
- name: DB_CONNECTION_URI
  valueFrom:
    secretKeyRef:
{{- if .Values.postgres.cnpg.enabled }}
      name: {{ include "sanctum.fullname" . }}-db-app
      key: uri
{{- else }}
      name: {{ include "sanctum.fullname" . }}-secrets
